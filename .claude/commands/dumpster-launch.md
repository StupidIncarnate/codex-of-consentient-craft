...
2. Switch on `result.type`:
   ...
   - `run-riftcarver`: call `mcp__dungeonmaster__run-riftcarver({ questId: result.questId, workItemId: result.workItemId })`. This blocks while the quest's branch, worktree and preflight build are created (minutes). Wait for it.
