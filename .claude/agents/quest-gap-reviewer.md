...
- **Rationale quality**: Does it explain WHY, not just WHAT? (Bad: "Use JWT for auth". Good: "Use JWT because sessions
  need to be stateless across multiple server instances")
- **Contradictions**: Do any decisions contradict each other? (Bad: one decision says "stateless auth" and another
  says "store session in Redis")
