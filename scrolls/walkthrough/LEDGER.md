...
| DEF-36 | SL · DEF-35 | After `dungeonmaster init` scaffolds `packages/hydration-recipes`, `siegelense recipes` fails until the consumer runs `npm install` and builds the new package. The user decided init runs both itself when it has just scaffolded the package | `queued` — the scaffold lives in `packages/siegelense`, where DEF-32 Piece 2 is working. Dispatch once Piece 2 merges |
