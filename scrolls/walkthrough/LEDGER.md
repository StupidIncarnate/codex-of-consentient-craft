...
| DEF-36 | SL · DEF-35 | After `dungeonmaster init` scaffolds `packages/hydration-recipes`, `siegelense recipes` fails until the consumer runs `npm install` and builds the new package. The user decided init runs both itself when it has just scaffolded the package | `queued` — waits for the `def-32-typefix` agent to leave `packages/siegelense` |
