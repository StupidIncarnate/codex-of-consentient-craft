...
| DEF-35 | SL · SL-002 | `dungeonmaster init` should add a `hydration-recipes` folder to a consumer repo that has none, so siegelense can seed from it | `dispatched — worktrees/def-35`. Init already creates an empty `packages/hydration-recipes/src/`, which nothing can build. The fix scaffolds a real package |
