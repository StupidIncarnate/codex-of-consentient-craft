...
**Valid as of the `storage` commit, 2026-09-18.** Re-derive the counts below before trusting them; the
...
| **Step verbs** | **18** | 23 | `goto` `waitFor` `click` `type` `screenshot` `eval` `look` `box` `seed` `until` `dom` `key` `health` `resize` `request` `before` `file` `storage` |
...
**`look`, `box`, `dom`, `seed`, `until`, `key`, `health`, `resize`, `request`, `before`, `file` and `storage` are built.** What is next is in "How to pick the next piece" — re-derive it
...
| 5 | `before` — a script ahead of the page's own | **done** — Playwright `addInitScript` via `initScriptAddLayerAdapter`, persists across navigations, reading `installed init script (N chars)`, refuses on headless; driven on real lane |
