// B8 — a TOP-LEVEL `filter`, in a plan holding two guilds. CONFIRM: a top-level filter has no
// `scope` and is instance-wide, matching D11 and `collection-chain-transformer.ts`'s own
// `ancestors.length === 0 ? undefined : ...` line. Reuses b4.ts's task-like ingredient (real
// query/remove routes over an in-memory array) so the filter+remove op actually has something to
// run against at tier 3.
import { z } from 'zod';
...
const dm = entryChainTransformer({
  ...
  guilds: { add: (count: number, build: (rows: readonly unknown[]) => readonly unknown[]) => readonly unknown[] };
  tasks: { filter: (a: { where: Record<PropertyKey, unknown> }) => { remove: () => unknown } };
};

const createOps = dm.guilds.add(GUILD_COUNT, (g: unknown) => {
...
// The TOP-LEVEL filter — no host, no scope. Confirms it matches BOTH guilds' tasks.
const filterOp = dm.tasks.filter({ where: { role: 'riftcarver' } }).remove();

const ops = [
  ...(createOps as unknown as readonly unknown[]),
  ...(filterOp as unknown as readonly unknown[]),
] as unknown as readonly unknown[];

const ingredients = [guildIngredient, questIngredient, taskIngredient] as unknown as readonly IngredientConfigData[];
const home = mkdtempSync(join(tmpdir(), 'dm-b8-'));

(async () => {
  console.log('tasksStore BEFORE run:', JSON.stringify(tasksStore));
  try {
    await planRunBroker({
      plan: { recipeName: 'b8-scratch', ops } as unknown as HydrationPlan,
