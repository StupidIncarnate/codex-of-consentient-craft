...
| sync caller page load race (`createSessionFile`, `createSessionWithAssistantText`) | 153 e2e | synchronous caller seam (racing `page.goto` against async `run()` route) | keep `sessionHarness.createSessionFile`, `sessionHarness.createSessionWithAssistantText`. Survey finding §14 Batch 10.10 & 10.11 |
...
| methods still writing directly | all of them | **12**, each named in §7 (`writeQuestFile`, `seedInProgressWithOperations`, `writeUnparseableQuestFile`, `writeWardResultDetail`, `patchQuestStatus`, `rewindQuestStatus`, `appendMainSessionLine`, `appendSubagentLine`, `createSessionFile`, `createSessionWithAssistantText`, plus assertions) |
