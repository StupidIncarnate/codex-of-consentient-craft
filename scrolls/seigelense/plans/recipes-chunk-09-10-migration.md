...
|---|---|---|
| `createSessionFile` | **139** | **STAYS** | synchronous caller seam (racing page.goto against async run() route). Survey finding §14 Batch 10.10 |
| `cleanSessionDirectory` / `cleanSessionFiles` | **22** / **3** | **STAYS** \| teardown isolates outside guild directory; guild API sweep cannot clean unlisted guild sessions. Survey finding §14 Batch 10.12 |
| `createSessionWithAssistantText` | **14** | **STAYS** \| synchronous caller seam (racing page.goto against async run() route). Survey finding §14 Batch 10.11 |
| `createSubagentTailOnly` | **6** | **STAYS** \| synchronous caller seam (racing async run() route). Survey finding §14 Batch 10.12 |
| `appendSubagentLine` / `appendMainSessionLine` | **2** / **5** | **STAY** — a bare id from outside the plan. Survey finding 2 |
| `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles` | **1 each** | **STAY** \| synchronous caller seam (racing async run() route). Survey finding §14 Batch 10.12 |
| `createNestedSubagentSessionFiles` | **1** | **STAYS** \| extra withNestedChain hardcodes task/prompt text; cannot express test marker assertions. Survey finding §14 Batch 10.13 |
| `createSubagentTailMultiEntry` | **2** | **STAYS** \| synchronous caller seam (racing async run() route). Survey finding §14 Batch 10.12 |
...
| sync caller page load race (`createSessionFile`, `createSessionWithAssistantText`, `createSubagentTailOnly`, `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles`, `createSubagentTailMultiEntry`) | 164 e2e | synchronous caller seam (racing `page.goto` against async `run()` route) | keep `sessionHarness.createSessionFile`, `sessionHarness.createSessionWithAssistantText`, `createSubagentTailOnly`, `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles`, `createSubagentTailMultiEntry`. Survey finding §14 Batch 10.10, 10.11 & 10.12 |
| `withNestedChain` text mismatch | 1 e2e | `withNestedChain` hardcodes task/prompt text; cannot express test marker assertions | keep `sessionHarness.createNestedSubagentSessionFiles`. Survey finding §14 Batch 10.13 |
...
| methods still writing directly | all of them | **20**, each named in §7 (`writeQuestFile`, `seedInProgressWithOperations`, `writeUnparseableQuestFile`, `writeWardResultDetail`, `patchQuestStatus`, `rewindQuestStatus`, `appendMainSessionLine`, `appendSubagentLine`, `createSessionFile`, `createSessionWithAssistantText`, `cleanSessionDirectory`, `cleanSessionFiles`, `createSubagentTailOnly`, `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles`, `createSubagentTailMultiEntry`, `createNestedSubagentSessionFiles`, plus assertions) |
...
| **10.10** | `sessionHarness.createSessionFile` (`session-without-quest.e2e.ts`) | synchronous caller page load race & missing target `home` | `ZodError: Required` inside `dmTargetContract.parse` | **Original:**<br>`e2e         @dungeonmaster/web   PASS  1 files, 123 discovered (5.5s)`<br>`run: 1789811957616-ed0f  (5.7s)`<br>`e2e:       PASS  1 packages (1 files passed/0 files failed, 123 discovered)  5.5s`<br>**Converted:**<br>`FAIL "Session without Quest (read-only view) › VALID: orphan session URL renders chat history without input or right panel"`<br>`ZodError: [`<br>`    {`<br>`    "code": "invalid_type",`<br>`    "expected": "string",`<br>`    "received": "undefined",`<br>`    "path": [`<br>`    "home"`<br>`    ],`<br>`    "message": "Required"`<br>`    },`<br>`    {`<br>`    "code": "invalid_type",`<br>`    "expected": "string",`<br>`    "received": "undefined",`<br>`    "path": [`<br>`    "claudeHome"`<br>`    ],`<br>`    "message": "Required"`<br>`    }`<br>`    ]`<br>`    271 \|     const jsonlPath = path.join(jsonlDir, ${sessionId}.jsonl);`<br>`    272 \|`<br>`    > 273 \|     const writeTarget = dmTargetContract.parse({ targetId: jsonlPath, writeMode: 'truncate' });`<br>`    \|                                          ^`<br>Per §8, `createSessionFile` remains raw and joins §7's table. |
| **10.11** | `sessionHarness.createSessionWithAssistantText` (`subagent-duration-session-blank.e2e.ts`) | synchronous caller page load race & target `write` error | `HydrationWriteFailedError` / `Error: page.goto: Test ended.` | **Original:**<br>`e2e         @dungeonmaster/web   PASS  1 files, 123 discovered (5.8s)`<br>`run: 1789812244951-0436  (6.0s)`<br>`e2e:       PASS  1 packages (1 files passed/0 files failed, 123 discovered)  5.8s`<br>**Converted:**<br>`FAIL "A session-transcript sub-agent chain with no landed completion notification renders no duration figure › EMPTY: {session transcript holding an assistant text reply and no Task tool-use line at all} => renders SUBAGENT_CHAIN count 0 and subagent-chain-duration count 0"`<br>`HydrationWriteFailedError: recipe "test-recipe": ingredient "session"'s write route failed writing "(unknown path)": [`<br>`    {`<br>`    "code": "custom",`<br>`    "message": "Path must be absolute (start with / or C:\\ on Windows)",`<br>`    "path": []`<br>`    }`<br>`    ]`<br>`    Error: page.goto: Test ended.`<br>`    Call log:`<br>`    - navigating to "http://dungeonmaster.localhost:39005/753ec977-64ec-4850-b257-d27d7c8cbcef/session/e2e-sd-blank-no-task-line", waiting until "load"`<br>Per §8, `createSessionWithAssistantText` remains raw and joins §7's table. |
| **10.12** | `sessionHarness` sub-agent family; `cleanSessionDirectory`/`cleanSessionFiles` (`chat-replay-subagent-grouping.e2e.ts`) | synchronous caller seam & missing target `home` / teardown isolation | `ZodError: Required` inside `dmTargetContract.parse` | **Original:**<br>`e2e         @dungeonmaster/web   PASS  1 files, 123 discovered (6.8s)`<br>`run: 1789812646643-65dc  (7.1s)`<br>`e2e:       PASS  1 packages (1 files passed/0 files failed, 123 discovered)  6.8s`<br>**Converted:**<br>`FAIL "Replay sub-agent grouping (file-sourced: main JSONL + subagent JSONL on disk) › VALID: {main JSONL has Agent tool_use + completion tool_use_result.agentId; subagent JSONL has sub-agent activity} => sub-agent chain groups its sub-agent file entries under the Task chain header on session reload"`<br>`ZodError: [`<br>`    {`<br>`    "code": "invalid_type",`<br>`    "expected": "string",`<br>`    "received": "undefined",`<br>`    "path": [`<br>`    "home"`<br>`    ],`<br>`    "message": "Required"`<br>`    },`<br>`    {`<br>`    "code": "invalid_type",`<br>`    "expected": "string",`<br>`    "received": "undefined",`<br>`    "path": [`<br>`    "claudeHome"`<br>`    ],`<br>`    "message": "Required"`<br>`    }`<br>`    ]`<br>`    310 \|`<br>`    311 \|     const jsonlPath = path.join(jsonlDir, ${sessionId}.jsonl);`<br>`    > 312 \|     const writeTarget = dmTargetContract.parse({ targetId: jsonlPath, writeMode: 'truncate' });`<br>`    \|                                          ^`<br>Per §8, sub-agent family methods and teardown remain raw and join §7's table. |
| **10.13** | `sessionHarness.createNestedSubagentSessionFiles` (`nested-subagent-rendering.e2e.ts`) | hardcoded `withNestedChain` text mismatch | `expect(locator).toBeVisible() failed` for `getByTestId('SUBAGENT_CHAIN_HEADER').filter({ hasText: 'Parent agent does work' }).first()` | **Original:**<br>`e2e         @dungeonmaster/web   PASS  1 files, 123 discovered (6.1s)`<br>`run: 1789812930736-2fde  (6.4s)`<br>`e2e:       PASS  1 packages (1 files passed/0 files failed, 123 discovered)  6.1s`<br>**Converted:**<br>`FAIL "Nested sub-agent renders recursively (live streaming + reload replay) › VALID: {reload: 3 nested JSONL files seeded on disk, page navigated to the quest} => replay renders the same recursive nested chain structure with the marker visible"`<br>`Error: expect(locator).toBeVisible() failed`<br>`Locator: getByTestId('SUBAGENT_CHAIN_HEADER').filter({ hasText: 'Parent agent does work' }).first()`<br>`Expected: visible`<br>`Error: element(s) not found`<br>`    286 \|     await expect(`<br>`    287 \|       page.getByTestId('SUBAGENT_CHAIN_HEADER').filter({ hasText: PARENT_DESCRIPTION }).first(),`<br>`    > 288 \|     ).toBeVisible({ timeout: CHAIN_TIMEOUT });`<br>`    \|       ^`<br>Per §8, `createNestedSubagentSessionFiles` remains raw and joins §7's table. |
| **10.14** | `catalogue recipes` (`guild-mid-execution`, `session-with-nested-chain`) | fixed doc-example fixtures & zero `expect(` diff invariant | tightly coupled setups demand assertion rewrites; new specs introduce new `expect(` lines | **Original:** PASS (verified via integration tests and CLI/MCP layer in Chunk 8).<br>**Converted:** FAIL — The catalogue recipes mint fixed, doc-example domain state (3 quests with riftcarver dropped, and `{"type":"init"}` session transcript). Existing e2e specs have tightly coupled setups and assertions that cannot be satisfied by these fixed doc fixtures without changing assertions. Creating new e2e specs would introduce new `expect(` lines, violating the invariant. Per §8, these catalogue recipes remain verified in their dedicated integration tests (`guild-mid-execution-recipe-broker.integration.test.ts` and `session-with-nested-chain-recipe-broker.integration.test.ts`) and at the CLI/MCP layer in Chunk 8. |
