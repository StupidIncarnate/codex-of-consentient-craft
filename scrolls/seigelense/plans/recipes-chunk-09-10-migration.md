...
| sync caller page load race (`createSessionFile`) | 139 e2e | synchronous caller seam (racing `page.goto` against async `run()` route) | keep `sessionHarness.createSessionFile`. Survey finding §14 Batch 10.10 |
...
| methods still writing directly | all of them | **11**, each named in §7 (`writeQuestFile`, `seedInProgressWithOperations`, `writeUnparseableQuestFile`, `writeWardResultDetail`, `patchQuestStatus`, `rewindQuestStatus`, `appendMainSessionLine`, `appendSubagentLine`, `createSessionFile`, plus assertions) |
