...
| `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles`, `createNestedSubagentSessionFiles` | **1 each** | `createNestedSubagentSessionFiles` CONVERTS; the rest **STAY** \| synchronous caller seam (racing async run() route). Survey finding §14 Batch 10.12 |
...
| methods still writing directly | all of them | **19**, each named in §7 (`writeQuestFile`, `seedInProgressWithOperations`, `writeUnparseableQuestFile`, `writeWardResultDetail`, `patchQuestStatus`, `rewindQuestStatus`, `appendMainSessionLine`, `appendSubagentLine`, `createSessionFile`, `createSessionWithAssistantText`, `cleanSessionDirectory`, `cleanSessionFiles`, `createSubagentTailOnly`, `createSubagentSessionFiles`, `createInFlightSubagentSessionFiles`, `createMultiSubagentSessionFiles`, `createSubagentTailMultiEntry`, plus assertions) |
