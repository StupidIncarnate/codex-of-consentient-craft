...
|----------------------------|------------|-------------|-------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| `codeweaver`               | N chunks   | agent       | `[pathseeker-walk id] + inter-step cw deps`                       | Steps chunked per package by `stepsToPackageChunksTransformer`; a chunk depends on the chunk(s) its steps depend on                  |
...
| `lawbringer`               | 1 per pkg  | agent       | `[all siege ids]` (or `[ward id]` if no flows)                    | Per-package chunking like codeweaver, filtered to reviewable source pairs (operational/barrel/package.json + flowrider-owned steps excluded); each parent fans out `lawbringer-minion` sub-agents per pair-group |
...
| **Lawbringer**   | One package chunk of reviewable file pairs; OR, in **whole-diff mode** (bug-hunt), just `questId` and no step refs | Orchestrates review: partitions its pairs and fans out `lawbringer-minion` sub-agents (review AND fix per pair-group: logic / error-handling / security + manual test branch-coverage walk + it.each cleanup), reads their artifacts, runs one batch ward, fixes remaining red, commits. Whole-diff mode reviews every changed file in the diff vs the repo default branch instead of named pairs | `complete` / `failed` (→ BLOCK)                  |
