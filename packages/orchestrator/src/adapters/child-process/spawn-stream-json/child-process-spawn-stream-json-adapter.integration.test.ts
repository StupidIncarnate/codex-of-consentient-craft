 ...
 *
 * The second SPAWNS A REAL CLAUDE CLI, because only a live session answers the other half: that
 * Claude Code honours an inline `--settings` blob and fires every hook in it ahead of the first
 * user entry. It measures off the transcript Claude Code itself writes rather than asking the
 * spawned model to self-report on its own system prompt — a self-report measured wrong roughly 1
 * time in 6.
 *
 * THAT SECOND TEST COSTS A BILLED MODEL TURN ON EVERY RUN and needs network plus a logged-in
...
// A live turn plus the harness's 15s transcript-flush poll. A per-test budget rather than a
// file-wide jest.setTimeout, so the hermetic test keeps jest's ordinary timeout.
const LIVE_TEST_TIMEOUT_MS = ElapsedMsStub({ value: 65_000 });
...
describe('childProcessSpawnStreamJsonAdapter integration', () => {
  ...
  it(
    'VALID: {trivial prompt, --settings hooks} => every configured SessionStart snippet attaches once, before the first user entry',
    async () => {
