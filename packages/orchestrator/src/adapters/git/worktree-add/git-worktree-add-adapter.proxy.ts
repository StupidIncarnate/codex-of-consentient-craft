...
// gitWorktreeAddAdapter always spawns bare `git`, matching the command childProcessSpawnCaptureAdapter
// is invoked with, so the underlying spawn mock's command-addressed staging matches. There is only
// ONE call per adapter invocation (`git worktree add ...`), so composing the shared
// childProcessSpawnCaptureAdapterProxy directly is enough; no raw `spawn` mocking or onceFor
// sequencing is needed.
const GIT_COMMAND = 'git';

export const gitWorktreeAddAdapterProxy = (): {
  setupSuccess: () => void;
