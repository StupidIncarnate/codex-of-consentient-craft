/**
 * PURPOSE: Executes the lawbringer phase within the orchestration loop — one agent per file pair with retries
 *
 * USAGE:
 * await runLawbringerLayerBroker({questFilePath, startPath});
 * // Spawns lawbringer agents for each file pair, retries failed ones
 */
...
export const runLawbringerLayerBroker = async ({
  ...
  if (_retryState !== undefined) {
    ...
    return runLawbringerLayerBroker({
      questFilePath,
      startPath,
      _retryState: {
