...
describe('questModifyBroker', () => {
  ...
  describe('chaoswhisperer pending gate on approved transition', () => {
    it('INVALID: {status: review_observables -> approved, chaoswhisperer pending} => returns success:false error mentioning chaoswhisperer', async () => {
      const proxy = questModifyBrokerProxy();
      const flow = FlowStub({
        nodes: [FlowNodeStub({ observables: [FlowObservableStub()] })],
      });
      ...
      expect(result).toStrictEqual({
        success: false,
        error:
          'Cannot transition to approved: chaoswhisperer work item(s) still in progress. Wait for ChaosWhisperer (and gap-minion) to finish.',
      });
      expect(proxy.getAllPersistedContents()).toStrictEqual([]);
    });

    it('INVALID: {status: review_observables -> approved, chaoswhisperer in_progress} => returns success:false', async () => {
      const proxy = questModifyBrokerProxy();
      const flow = FlowStub({
        nodes: [FlowNodeStub({ observables: [FlowObservableStub()] })],
      });
    ...
    it('VALID: {status: review_observables -> approved, chaoswhisperer complete} => returns success:true', async () => {
      const proxy = questModifyBrokerProxy();
      const flow = FlowStub({
        nodes: [FlowNodeStub({ observables: [FlowObservableStub()] })],
      });
      const chaosItem = WorkItemStub({
        id: 'cccccccc-3333-4333-8333-333333333333' as never,
        role: 'chaoswhisperer',
        status: 'complete',
      });
      const quest = QuestStub({
    ...
    it('VALID: {status: review_observables -> approved, chaoswhisperer skipped} => returns success:true', async () => {
      const proxy = questModifyBrokerProxy();
      const flow = FlowStub({
        nodes: [FlowNodeStub({ observables: [FlowObservableStub()] })],
      });
      const chaosItem = WorkItemStub({
        id: 'dddddddd-4444-4444-8444-444444444444' as never,
        role: 'chaoswhisperer',
        status: 'skipped',
      });
      const quest = QuestStub({
