...
describe('questPipelineBroker', () => {
  ...
  describe('abort signal already aborted', () => {
    it('EDGE: {abortSignal already aborted before start} => onPhaseChange called with failed, no phases run', async () => {
      questPipelineBrokerProxy();
      const processId = ProcessIdStub({ value: 'proc-test-9' });
      const questId = QuestIdStub({ value: 'add-auth' });
      const questFilePath = FilePathStub({ value: '/quests/quest.json' });
      const startPath = FilePathStub({ value: '/project/src' });
      const phases: OrchestrationPhase[] = [];
  ...
  describe('no abort signal provided', () => {
    it('VALID: {abortSignal omitted, all layers succeed} => pipeline completes normally', async () => {
      const proxy = questPipelineBrokerProxy();
      const processId = ProcessIdStub({ value: 'proc-test-10' });
      const questId = QuestIdStub({ value: 'add-auth' });
      const questFilePath = FilePathStub({ value: '/quests/quest.json' });
      const startPath = FilePathStub({ value: '/project/src' });
      const phases: OrchestrationPhase[] = [];
