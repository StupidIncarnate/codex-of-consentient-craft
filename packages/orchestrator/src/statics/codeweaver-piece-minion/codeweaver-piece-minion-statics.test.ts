import { codeweaverPieceMinionStatics } from './codeweaver-piece-minion-statics';

describe('codeweaverPieceMinionStatics', () => {
  it('VALID: exported value => has expected keys with string values', () => {
    expect(codeweaverPieceMinionStatics).toStrictEqual({
      prompt: {
  ...
  it('VALID: template => declares it is summoned by Codeweaver in the opening line', () => {
    const needle =
      'You are a codeweaver-piece-minion. Codeweaver summoned you (via the Agent tool) to implement ONE isolated piece of its slice';
    const { template } = codeweaverPieceMinionStatics.prompt;
  ...
  it('VALID: template => has a "What you return" distilled-artifact section', () => {
    expect(codeweaverPieceMinionStatics.prompt.template).toMatch(
  ...
  it('VALID: template => Briefing section ends with $ARGUMENTS placeholder', () => {
    expect(codeweaverPieceMinionStatics.prompt.template).toMatch(/^\$ARGUMENTS$/mu);
  });
