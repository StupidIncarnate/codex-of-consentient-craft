...
import { pesteaterWorkerMinionStatics } from './pesteater-worker-minion-statics';
...
const TEMPLATE = pesteaterWorkerMinionStatics.prompt.template;

describe('pesteaterWorkerMinionStatics', () => {
  ...
  it('VALID: served template => restates no block the worker information tool serves', () => {
    expect({
      ...
      gitBan: hasIn({
        needle: 'Leave your work in the tree and your REVIEWER commits the whole round',
        text: TEMPLATE,
      }),
  ...
  // WHAT STAYED IS WHAT ANOTHER WORKER WOULD READ AS FALSE. Nothing else on this quest reproduces a
  // bug before fixing it, so the red-from-unchanged-source discipline, the six non-reproductions, the
  // narrowest-fix rule and the diagnostic-probe exception all stay this file's own.
  it('VALID: served template => keeps every rule that reproducing a bug is alone in needing', () => {
    expect({
      redFromUnchangedSource: hasIn({
        needle: '**Your red comes from the real system misbehaving on UNCHANGED source**',
        text: TEMPLATE,
      }),
      actualExpectedLabels: hasIn({
        needle:
          '`ACTUAL: <symptom today>` and one labelled `EXPECTED: <what the fix must make real>`',
        text: TEMPLATE,
      }),
      sixNonReproductions: hasIn({
        needle: '**These six reds are NOT a reproduction:**',
        text: TEMPLATE,
      }),
      narrowestFix: hasIn({
        needle: '**Only now, fix it.** Apply the NARROWEST change that makes the observable true',
        text: TEMPLATE,
      }),
      diagnosticProbeException: hasIn({
        needle:
          '**A temporary diagnostic line is a different thing from a fix, and it has its own bounds.**',
        text: TEMPLATE,
      }),
    }).toStrictEqual({
      redFromUnchangedSource: true,
      actualExpectedLabels: true,
      sixNonReproductions: true,
      narrowestFix: true,
      diagnosticProbeException: true,
    });
  });

  // A NUMBERED STEP IS A CLAIM ABOUT ORDER, and this workflow broke that claim. Step 4 used to write
  // the test, run it against unchanged source, fix the bug AND check the ripple, while step 5 said
  // "read the failure output from step 4's run" — by which point the fix had already landed. The
  // giveaway was a forward reference inside step 4 reading "step 5 happens HERE", which is the served
  // text admitting its own numbering is wrong. Three halves are pinned, because each fails
  // differently: a gap or duplicate in the sequence, a back-reference past the last step, and any step
  // claiming another happens inside it.
  it('VALID: served template => numbers its steps in order and claims no step happens inside another', () => {
    const workflow = TEMPLATE.slice(TEMPLATE.indexOf('\n## Workflow'));
  ...
  // THE RED MUST COME FROM UNCHANGED SOURCE, so step 4 stopping before the implementation is the whole
  // evidential basis of this discipline — a line changed first makes the red prove nothing about the
  // bug. Pinned as the boundary plus its cost, and with the fix pinned OUT of step 4.
  it('VALID: served template => stops step 4 before the implementation, and fixes at step 6', () => {
    const workflow = TEMPLATE.slice(TEMPLATE.indexOf('\n## Workflow'));
    const stepFour = workflow.slice(workflow.indexOf('4. **'), workflow.indexOf('5. **'));

    expect({
      fourTouchesNoImplementation: hasIn({
        needle: '**Touch no implementation line in this step.**',
        text: stepFour,
      }),
      fourNamesTheCost: hasIn({
        needle: 'a line you changed first is a red that proves nothing about the bug',
        text: stepFour,
      }),
      sixIsTheFix: hasIn({ needle: '6. **Only now, fix it.**', text: TEMPLATE }),
      fourStillFixes: hasIn({ needle: '**Only now, fix it.**', text: stepFour }),
    }).toStrictEqual({
      fourTouchesNoImplementation: true,
      fourNamesTheCost: true,
      sixIsTheFix: true,
      fourStillFixes: false,
    });
  });

  // THE COLLISION RULE ITSELF IS THE TOOL'S, and this prompt spends the two hooks that text leaves
  // open. This discipline pays most for getting the boundary wrong: a traced cause sits WHERE IT
  // SITS, routinely somewhere the chunk never named, so a ban on any existing file told the one
  // session that had followed the symptom to its source to hand the source back unfixed. Pinned from
  // both sides — the shared sentences must NOT come back, and the local answers must stay.
  it('VALID: served template => answers the collision hooks without restating the rule', () => {
    expect({
      ...
      theCauseSitsWhereItSits: hasIn({
        needle:
          '**That open set is where a traced cause usually lands, and this round leans on it hardest.**',
        text: TEMPLATE,
      }),
      restatesTheTable: hasIn({ needle: '| The path | Why nothing collides |', text: TEMPLATE }),
      existingFileBanIsGone: hasIn({
        needle: '**An EXISTING file no chunk lists is one you must not change.**',
        text: TEMPLATE,
      }),
    }).toStrictEqual({
      defersToTheTool: true,
      nothingWidensIt: true,
      theCauseSitsWhereItSits: true,
      restatesTheTable: false,
      existingFileBanIsGone: false,
    });
  });

  // The shared payload lists FOUR `rework` triggers and says a prompt adds more. These are the ones it
  // adds — "you could not reproduce the bug as described" chief among them, since nothing else on this
  // quest even attempts a repro. A reader that met only the four would swallow exactly these, so their
  // presence here is what makes that split honest.
  it('VALID: served template => adds this round`s own `rework` triggers to the shared four', () => {
    expect({
      ...
      couldNotReproduce: hasIn({
        needle: 'You could not reproduce the bug as described.',
        text: TEMPLATE,
      }),
