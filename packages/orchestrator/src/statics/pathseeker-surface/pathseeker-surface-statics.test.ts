import { pathseekerSurfaceStatics } from './pathseeker-surface-statics';

describe('pathseekerSurfaceStatics', () => {
  it('VALID: exported value => has expected keys with string values', () => {
    expect(pathseekerSurfaceStatics).toStrictEqual({
      prompt: {
  ...
  describe('prompt.template content', () => {
    it('VALID: template => declares slice-authoritative deliverable in opening paragraph', () => {
      expect(pathseekerSurfaceStatics.prompt.template).toMatch(
        /^You are a pathseeker-surface agent\. The orchestrator monitor dispatched you with an assigned slice of a quest spec\. Your job is to commit the steps\[\] and contracts\[\] for your slice directly to the quest via modify-quest\. You are the authority for your slice's step shape — you write the steps yourself\.$/mu,
      );
    });

    it('VALID: template => Constraints section opens with Scope bucket', () => {
      expect(pathseekerSurfaceStatics.prompt.template).toMatch(/^\*\*Scope:\*\*$/mu);
    });
