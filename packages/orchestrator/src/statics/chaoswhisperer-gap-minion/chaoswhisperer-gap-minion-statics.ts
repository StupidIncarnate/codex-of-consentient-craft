...
- **Existing contract verification**: For contracts marked as \`existing\`, verify they actually exist in the codebase with the declared shape. This is the ONE case where codebase search is required — you are validating a claim the spec makes. Use \`get-project-inventory({ packageName })\` to find the contract by name (NOT \`discover\` with a glob — naming variants like \`email/\` vs \`email-address/\` vs \`user-email/\` make globs miss), then \`Read\` the contract file to check the shape against the spec. For contracts NOT marked as \`existing\`, they are new and will be created during implementation.
