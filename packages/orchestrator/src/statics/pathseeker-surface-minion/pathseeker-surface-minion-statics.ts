...
- **Never author a ward / quality-gate step.** Do NOT write a step whose job is to run ward, lint, typecheck, tests, or \`npm run build\` for gating (e.g. focus \`[command] npm run ward … expect exit 0\` or \`[verification] lint + typecheck + tests pass\`). Ward is baked into the workflow: a \`changed\`-scope ward floor runs automatically after the Codeweaver chain and a \`full\` ward floor runs at the very end, with failures auto-routed to fixer agents. A ward/build step only adds a redundant floor that re-runs what those floors already run. If the spec handed you a "ward exits 0" / "build exits 0" observable, do NOT create a step for it — claim it step-level on the nearest real implementation step in your slice (its code must pass ward anyway, and the baked-in floors actually run it) so the coverage validator is satisfied, and note it in your final message as a redundant observable.
...
- Each flow's \`flowType\` field — \`runtime\` flows assert runtime behavior (user clicks, API hits); \`operational\` flows assert post-execution state (grep zero-matches, directory gone, file exists)
...
**Consolidate confirm-post-state checks into ONE \`focusAction\` step.** A \`focusAction\` step has no \`focusFile\`, so the work-item builder gives each one its OWN chunk → its own agent floor. If your slice has several observables that only *confirm* post-execution state (a directory is gone, a grep returns zero, a file exists, a symbol is absent), do NOT author one \`focusAction\` step per observable — that spawns a wasted full agent per check. Author a SINGLE \`focusAction: { kind: 'verification' }\` step that carries one assertion per check, each assertion claiming its own observable via \`observablesSatisfied\`, so one agent runs every confirmation in a single pass. Steps that do REAL work (create / modify / delete a specific file) stay separate — this only merges the pure post-state confirmations.
...
- **Verification consolidation + no ward step.** If you authored more than one \`focusAction\` step that only confirms post-state (directory-gone, grep-zero, file-exists, symbol-absent), collapse them into ONE \`focusAction: { kind: 'verification' }\` step (one assertion per check, each claiming its observable) — every separate one becomes its own wasted agent floor. AND confirm you authored NO step whose job is to run ward / lint / typecheck / tests / \`npm run build\` — those run automatically as dedicated floors; delete any such step before commit.
