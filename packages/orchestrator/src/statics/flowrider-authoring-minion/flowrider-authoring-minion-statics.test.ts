...
import { flowriderAuthoringMinionStatics } from './flowrider-authoring-minion-statics';

const has = (needle: string): boolean =>
  flowriderAuthoringMinionStatics.prompt.template.includes(needle);

describe('flowriderAuthoringMinionStatics', () => {
  it('VALID: exported value => has expected keys with string values', () => {
    expect(flowriderAuthoringMinionStatics).toStrictEqual({
      prompt: {
  ...
  it('VALID: template => carries exactly one $ARGUMENTS token on its own line', () => {
    const { template } = flowriderAuthoringMinionStatics.prompt;
  ...
  it('VALID: template => ends with the Briefing section the operator fills', () => {
    expect(flowriderAuthoringMinionStatics.prompt.template).toMatch(/^## Briefing$/mu);
  });
  ...
  // Mode C's deliverable is a predicate RESULT, not a suite. Told only that Mode C exists, a minion
  // writes a test whose assertion re-states the predicate and reports it as coverage — and the
  // edges it never walked read as walked.
  it('VALID: template => makes Mode C a verification rather than a test suite', () => {
    expect({
      verificationNotASuite: has('**Modality: VERIFICATION, not a test suite.**'),
      runsThePredicateAsWritten: has(
        'Run the predicate exactly as written and record the\nreal result.',
      ),
      authorsNoTestAndWalksNoEdges: has('You author no test for these and walk no edges.'),
    }).toStrictEqual({
      verificationNotASuite: true,
      runsThePredicateAsWritten: true,
      authorsNoTestAndWalksNoEdges: true,
    });
  });
  ...
  // Shape and layer answer different questions. Read as alternatives, a minion picks one and drops
  // the other — a "journey" e2e that never asserts where the claim lives, or a matrix that flattens
  // a branchy flow into a single parameterized case that walks no path.
  it('VALID: template => makes journey/matrix choose the SHAPE and checkSurface choose the LAYER', () => {
    expect({
      composeNotCompete: has(
        '**Two more rules compose on top of that, and they answer different questions — never treat them as\nalternatives.**',
      ),
      shapeIsJourneyOrMatrix: has(
        'JOURNEY-vs-MATRIX chooses the test SHAPE: how many tests there are and what each one\nwalks.',
      ),
      layerIsCheckSurface: has(
        '`checkSurface` chooses the LAYER: where each assertion inside that shape reads its value\nfrom.',
      ),
      branchyFlowIsAJourney: has(
      ...
      neitherOverridesTheOther: has(
        'Picking a shape never licenses asserting at the wrong layer, and picking a layer never flattens a\nbranchy flow into one parameterized case.',
      ),
    }).toStrictEqual({
      composeNotCompete: true,
      shapeIsJourneyOrMatrix: true,
      layerIsCheckSurface: true,
      branchyFlowIsAJourney: true,
      journeyRendersPerSurface: true,
      matrixIsIntegration: true,
      neitherOverridesTheOther: true,
    });
  });

  // With `track` threaded through, remainingItemIds is a real gate count — but it is the OPERATOR's,
  // and it is measured against sign-offs an independent audit writes. A minion that signed its own
  // work would satisfy that gate the moment it returned.
  it('VALID: template => reads remainingItemIds as the operator’s gate count and signs nothing itself', () => {
    expect({
      isThePerTrackDifference: has(
        "`remainingItemIds` is the per-track sign-off difference, which is your OPERATOR's\ngate count and not your scope",
      ),
      authorsAgainstItems: has('you author against every id in `items`'),
      signsNothing: has('**you sign nothing at\nall.**'),
      auditWritesTheTrack: has(
        'An independent `flowrider-coverage-minion` writes the Flowrider track after you',
      ),
      namesTheSelfGradingHazard: has(
        'a\nsignature from the session that wrote the test would satisfy the gate the moment you returned',
      ),
    }).toStrictEqual({
      isThePerTrackDifference: true,
      authorsAgainstItems: true,
      signsNothing: true,
      auditWritesTheTrack: true,
      namesTheSelfGradingHazard: true,
    });
  });
  ...
  it('VALID: template => defines the artifact block with a FAILS IF field per observable', () => {
    expect({
      hasArtifactHeading: /^## Your Artifact/mu.test(
        flowriderAuthoringMinionStatics.prompt.template,
      ),
  ...
  // A red test proving a defect and an item no layer can reach are opposite evidentiary states, and
  // the artifact keeps a separate block for each. Collapsing them tells the operator to hand-check
  // something a failing test already proves.
  it('VALID: template => routes a handed-up defect to DEFECTS LEFT UNFIXED, never to the blocked block', () => {
    expect({
      ...
      blockedBlockSaysNoTestExists: has(
        'BLOCKED BY MISSING INFRASTRUCTURE (no layer available to me can prove this — no test exists):',
      ),
      selfAuditNamesTheDifference: has(
        'a `DEFECT:` has a red test\nproving it, while an item blocked by missing infrastructure has no test because no layer available to\nyou can reach it',
      ),
    }).toStrictEqual({
      handsUpToTheDefectBlock: true,
      defectBlockSaysProvenRed: true,
      blockedBlockSaysNoTestExists: true,
      selfAuditNamesTheDifference: true,
    });
  });

  // Unfinished scope banked in the blocked block reads to the operator as "covered as well as it
  // can be", so the bundle looks complete and nobody ever comes back for it.
  it('VALID: template => reports unreached scope as its own block rather than banking it elsewhere', () => {
    expect({
      notReachedBlock: has(
        'NOT REACHED (bundle scope I did not get to — neither covered nor blocked by anything):',
      ),
      selfAuditForbidsBanking: has('Neither is a place to put an item you simply did not get to'),
      ...
      auditsTerminalsAndBranchesToo: has(
        "**Every item the checklist returned** must leave this step in exactly\none of your artifact's blocks",
      ),
      settlesNothingItself: has('**You settle nothing.**'),
  ...
  // Observed live: when `get-agent-prompt` rejected 'flowrider-authoring-minion' (stale enum on the running
  ...
  it('VALID: template => is substantial enough to carry the authoring methodology', () => {
    expect(flowriderAuthoringMinionStatics.prompt.template.length).toBeGreaterThan(2000);
  });
});
