...
const backgroundTaskRule = `**2. NEVER end your turn waiting for a background task, and NEVER poll for one.** Run commands in the foreground and let them finish before you continue. A sub-agent does NOT receive an async wakeup when a detached background task finishes — end your turn waiting on one and your work item hangs forever. The remedy is to keep every command short enough to finish in the foreground: a command the harness auto-backgrounds is a sign you ran the wrong (too-broad) one, so scope it down and re-run scoped rather than waiting on it or re-running it as-is. Never try to "wait it out": no \`sleep N && tail\`, no \`while pgrep …; do sleep; done\`, no re-reading the partial output file in a loop. (A \`pgrep -f "<term>"\` poll loop also matches its OWN command line — \`<term>\` is in the loop's argv — so it never exits and burns the entire timeout.) Ward is the usual offender: a minion or sub-agent that kicks off a broad \`npm run ward\` (whole-repo OR a bare \`-- packages/<pkg>\` directory) will watch it get auto-backgrounded and then hang forever awaiting a completion notification that never arrives — keep ward FILE-scoped (Rule 3). (CLAUDE.md ward rules: pick ONE mode, never the foreground/background hybrid.)`;

const wardScopeRule = `**3. Run ward SCOPED to what you changed, ALWAYS in the foreground. NEVER run the whole-repo \`npm run ward\`.** Always \`npm run ward -- --only <checks> -- <paths>\` with \`timeout: 600000\`, scoped to the files you touched — it stays in the foreground and finishes fast. Those \`<paths>\` MUST be explicit FILE paths (\`-- <file1> <file2>\`), NEVER a bare directory (\`-- packages/<pkg>\`): a directory scope pulls in the whole package and the harness auto-backgrounds it exactly like the whole-repo command, which strands your turn (see Rule 2). The full-repo regression sweep is the dispatcher's own \`run-ward\` work item that runs after you; your job is only to prove the files YOU changed are green.`;
