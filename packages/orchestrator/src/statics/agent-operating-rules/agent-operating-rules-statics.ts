 ...
 * | planner-minion | `turnEndMinion` | `wardScoped` | `delegationSpike` | `wallMinion` | — |
 * | worker-minion, reviewer-minion | `turnEndMinion` | `wardScoped` | `delegationLeafBan` | `wallMinion` | — |
 *
 ...
 *
 * AXIS 3 SPLITS THE WORK-ITEM FAMILY, on whether the reader runs ward. `wardScoped` tells its reader
 * how to scope a run. An operator runs none: its reviewer runs the round's single
 * `npm run ward -- --staged`. Hand an operator the scoping rule and you hand back a command its own
 * prompt puts in the FORBIDDEN table, so the operator prompt takes `wardNone` instead.
 *
...
const wardNone = `**[WARD] You run NO ward, NO test and NO check of any kind.** Your REVIEWER runs the round's ward, once, as \`npm run ward -- --staged\`. That is every check type over every source file origin does not have yet. That range IS this round, because you push once at the end of each round. This OVERRIDES both the \`<dungeonmaster-ward>\` and the \`<dungeonmaster-ward-discipline>\` snippets you were handed at session start. Neither is addressed to an operator. A ward you ran yourself would print output you cannot read, over files you cannot open. It would also compete with your reviewer for the same tree.`;
