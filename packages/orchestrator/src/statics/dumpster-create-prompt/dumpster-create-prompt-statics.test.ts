...
// line-anchored `toMatch` assertions read `dumpsterCreatePromptStatics.prompt.template` directly instead.
...
describe('dumpsterCreatePromptStatics', () => {
  ...
  describe('node package tagging', () => {
    ...
    it('VALID: prompt template => the "does NOT map observables to file paths" boundary is unchanged and not contradicted', () => {
      const needle =
        '**Does NOT:** - Map observables to file paths (Codeweavers decide files at build time) - Write actual code - Read files directly (exploration sub-agents only) - Define file names, folder structure, or code organization - Write raw mermaid diagrams (mermaid is auto-generated from structured nodes/edges)';
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
    ...
    it('VALID: web login example flow => tags every node with packages and marks the two glue nodes with both sides of the seam', () => {
      const needle =
        '{ "id": "server-validates", "label": "Server validates?", "type": "decision", "packages": ["<ui-package>", "<api-package>"] }, { "id": "set-cookie", "label": "Set auth cookie", "type": "action", "packages": ["<ui-package>", "<api-package>"] },';
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
  ...
  // `source` paths (one codeweaver item per PACKAGE, carrying its flows and its contracts
  // together). There is no `packageNames`/`flowIds` gate for ChaosWhisperer to satisfy anymore;
  ...
  describe('derived implementation ledger', () => {
    it('VALID: prompt template => explains the ledger is derived from node package tags and contract source paths at Start', () => {
      const needle =
        'At Start the orchestrator DERIVES the implementation ledger from the node tags and contract sources — one codeweaver item per package, carrying its flows and its contracts together, ordered dependencies-first — appends the verify tail after it, and Codeweaver sessions relay through the items one at a time.';
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
    ...
    it('VALID: prompt template => says a seam node lands in BOTH packages’ items, so a missing tag loses that half', () => {
      const needle =
        "A node tagging TWO packages lands in BOTH their items — a seam has two halves and each side builds its own, in build-order — so a package you leave off a seam node loses its half of that node entirely, and the observables you attributed to it reach no session's scope.";
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
  ...
  describe('observable package attribution', () => {
    ...
    it('VALID: Observable Format => the single-package example omits package and says the save fills it in', () => {
      const needle =
        'On a node tagging exactly ONE package, leave `package` out — the save fills it in from the node: ```json { "id": "check-login-api-called", "type": "api-call", "description": "POST /api/auth/login called with credentials" } ```';
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
    ...
    it('VALID: Observable Format => the seam-node example carries an explicit package on every observable', () => {
      const needle =
        '"observables": [ { "id": "check-login-api-called", "type": "api-call", "description": "POST /api/auth/login called with credentials", "package": "<api-package>" }, { "id": "check-redirect-dashboard", "type": "ui-state", "description": "redirected to /dashboard", "package": "<ui-package>" } ]';
      const foundIndex = template.indexOf(needle);
      const foundSlice = template.slice(foundIndex, foundIndex + needle.length);
