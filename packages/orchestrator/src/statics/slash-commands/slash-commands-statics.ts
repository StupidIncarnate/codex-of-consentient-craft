...
2. Switch on \`result.type\`:
   ...
   - \`run-ward\`: call \`mcp__dungeonmaster__run-ward({ questId: result.questId, workItemId: result.workItemId })\`. This blocks while ward runs (minutes). Wait for it. The quest-close ward is the merge rung; nothing inside a pass runs it.
