 ...
 * actual-state and expected-state flows: a pair duplicates the whole reproduction path in both
 * flows, hides which step is the one that diverges, and gives a two-bug report no honest shape at
 ...
 * given/when/then block at all. An intake writing BDD prose was writing fields the contract drops
...
ward → blightscout → ward verify the fix.
