...
describe('pathseekerAssertionCorrectnessMinionStatics', () => {
  ...
  describe('prompt.template content', () => {
    ...
    it('VALID: template => Step 6 BLOCKED final message includes failedChecks verbatim', () => {
      const needle =
    ...
    it('VALID: template => Step 7 header returns final message ONCE', () => {
      expect(pathseekerAssertionCorrectnessMinionStatics.prompt.template).toMatch(
        /^### Step 7: Return Your Final Message ONCE$/mu,
      );
    });

    it('VALID: template => Step 7 complete final-message summary names the four fix counts and Ambiguous list', () => {
      const needle =
