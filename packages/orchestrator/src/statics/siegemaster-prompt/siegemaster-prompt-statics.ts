...
**[BACKGROUND] Never end your turn waiting for a background task, and never poll one.** Nothing wakes you when a detached background task finishes, so a turn that ends waiting on one hangs your work item for good. Keep every command short enough to finish in the foreground, with ONE exception: the dev server you start at step 2 and keep alive for the whole session. You never wait on that one and never poll it, so leaving it running in the background is correct and narrowing it is not. If the harness pushes any OTHER command into the background, you scoped it too broadly. Narrow it and run it again.
