import { pathseekerDedupStatics } from './pathseeker-dedup-statics';

describe('pathseekerDedupStatics', () => {
  it('VALID: exported value => has expected keys with string values', () => {
    expect(pathseekerDedupStatics).toStrictEqual({
      prompt: {
  ...
  describe('prompt.template content', () => {
    it('VALID: template => declares contract-dedup role in opening paragraph', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(
    ...
    it('VALID: template => Constraints section opens with Scope bucket', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^\*\*Scope:\*\*$/mu);
    });
    ...
    it('VALID: template => Scope bullet forbids Edit/Write/NotebookEdit against packages', () => {
      ...
      const { template } = pathseekerDedupStatics.prompt;
    ...
    it('VALID: template => declares modify-quest authority over steps[] and contracts[]', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^\*\*modify-quest authority:\*\*$/mu);
    });
    ...
    it('VALID: template => Workflow section header present', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^## Workflow$/mu);
    });
    ...
    it('VALID: template => Step 7 signal back', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^### Step 7: Signal Back$/mu);
    });
    ...
    it('VALID: template => Quest Context section header present', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^## Quest Context$/mu);
    });

    it('VALID: template => Quest Context section ends with $ARGUMENTS placeholder', () => {
      expect(pathseekerDedupStatics.prompt.template).toMatch(/^\$ARGUMENTS$/mu);
    });
  });
});
