...
5. **Do NOT redo committed work. Resume from the matching phase below, keyed on what is present:**
   ...
   - \`scopeClassification\` present but **auto-seeded** — every slice in \`scopeClassification.slices\` has an empty \`flowIds\` array (\`flowIds: []\`) — → treat it as un-classified and run **Phase 1: Scope** to re-validate/re-slice before summoning Wave A. \`orchestration-start-responder\` seeds this scope 1:1 from \`quest.packagesAffected\` (one slice per package, marker text in its \`slicing\` field: "Auto-generated from quest.packagesAffected") without ever consulting the flow/observable map, so Phase 1 must re-size each slice against the scope rubric, split or budget-scale heavy slices, populate \`flowIds\` from the flow/observable map, and overwrite \`scopeClassification\` before Wave A runs.
