...
\`signal-back\`, and your work item stays \`in_progress\` for good. A turn you end while a HELPER is
still out is a different thing — see [HELPERS]. A turn you end while a COMMAND is still running KILLS
that command — see the ward-discipline snippet.
...
It reads the quest, reads git, opens every test the pass wrote, judges whether each one bites, wards,
fixes what it can, and commits.
...
**Never argue with a \`rework\`.** It opened the tests and ran ward; you read a diff.
