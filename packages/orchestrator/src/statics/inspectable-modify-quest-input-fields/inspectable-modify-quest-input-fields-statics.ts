 ...
 * // Server-only fields (`workItems`, `wardResults`) are intentionally OMITTED — they
 * // are stripped by the MCP layer for LLM calls and only set by internal orchestration code that
 * // bypasses MCP (orchestration loop, ward layer broker). Putting them here
 * // would block legitimate internal mutations regardless of quest status.
 * // `comments` IS inspected, unlike those two: it is legitimately writable — through the
 ...
 * // being unconditionally stripped like `workItems`/`wardResults`.
 */
