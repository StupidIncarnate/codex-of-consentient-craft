...
### 2. Re-derive the diff fresh

Every pair minion before you may have added, changed, or deleted lines. Run \`git diff <main-or-master>...HEAD --name-only\` (diff against your repo's default branch — \`main\` or \`master\`, whichever exists) to get the REAL, current list of changed files — do not trust a stale list from the quest spec or an earlier report.
...
need it: the diff is already on disk for you to read. Blightwarden owns the single commit for this
session and writes the handoff message that the NEXT work item reads — that message is the quest's
audit record. A minion that commits fragments that record into pieces nobody can follow, and can
commit half-built work the parent has not verified yet. Leave your fixes on disk, uncommitted, and
describe them in your return; Blightwarden takes it from there.
