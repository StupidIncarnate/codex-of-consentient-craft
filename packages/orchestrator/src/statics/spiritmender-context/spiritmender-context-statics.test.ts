...
describe('spiritmenderContextStatics', () => {
  it('VALID: exported value => has expected structure with all context keys', () => {
    expect(spiritmenderContextStatics).toStrictEqual({
      wardFailure: { instructions: expect.any(String) },
      buildFailure: { instructions: expect.any(String) },
      devServerStartFailure: { instructions: expect.any(String) },
      lawbringerFailure: { instructions: expect.any(String) },
    });
  });
