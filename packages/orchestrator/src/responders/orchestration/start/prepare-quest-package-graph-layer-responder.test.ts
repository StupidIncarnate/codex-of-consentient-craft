...
describe('PrepareQuestPackageGraphLayerResponder', () => {
  ...
  describe('this workspace, derived from its own manifests', () => {
    it("VALID: {every packages/* manifest on disk, read verbatim} => each package's depth is the layer its real dependencies put it in", async () => {
      ...
      //   L0 testing / L1 shared / L2 config, eslint-plugin, hooks, session-forensics,
      //   siegelense, siegelense-recipes, tooling, web / L3 local-eslint, orchestrator, ward /
      ...
      // (L0) devDependency. `siegelense-recipes` carries that same shape — a
      // `@dungeonmaster/shared` dependency plus a `@dungeonmaster/testing` devDependency — so it
      // is L2 as well.
