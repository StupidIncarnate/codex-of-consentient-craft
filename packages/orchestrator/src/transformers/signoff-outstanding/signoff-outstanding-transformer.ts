 ...
 *
 * TWO INDEPENDENT TRACKS, ONE FUNCTION. A `flowrider` item is measured against `flowriderSignoff`,
 * a `siegemaster` item against `siegemasterSignoff`. Neither reads the other's field, so the same
 * unit can be outstanding for one track and settled for the other. Every role other than those two
 * returns an empty array and is never gated here.
 *
 * SCOPE IS PER-ROLE, AND THE ROLE IS THE ONLY GATE CONDITION.
 * - `flowrider` is gated UNCONDITIONALLY on role. Its denominator is computed HERE, at gate time,
 *   as the quest's flows of the types `signoffTrackEligibilityStatics` gives that track — never
 *   from `operationItem.flowIds`. Reading `flowIds` would reopen the exact hole this closes: an
 *   item declaring none would be ungated, and a whole-quest flowrider item legitimately carries
 *   none. Zero eligible flows yields zero units for the right reason (there is nothing walkable to
 *   prove), not because the gate was skipped.
 ...
 *
 * This file owns SCOPE — which flows an item is measured over. Which units within a flow are
...
export const signoffOutstandingTransformer = ({
  ...
  if (operationItem.role !== 'flowrider' && operationItem.role !== 'siegemaster') {
    return [];
  }

  const track = operationItem.role;

  // Which FLOW TYPES a track measures is data in `signoffTrackEligibilityStatics`, alongside the
  // unit kinds and observable origins it owns — one file answers "what is in this track's
  // denominator" rather than that rule being half data and half a branch here.
  const eligibleFlowTypes = new Set(
    signoffTrackEligibilityStatics.byTrack[track].flowTypes.map(String),
  );
  const typedFlows = quest.flows.filter((flow) => eligibleFlowTypes.has(flow.flowType));

  // Flowrider's denominator is every flow of an eligible type, resolved now. Siegemaster's is the
  // item's declared flows. An empty declared set matches no flow, which is how a siegemaster item
  ...
  return scopedFlows.flatMap((flow) => signoffFlowOutstandingTransformer({ flow, track }));
};
