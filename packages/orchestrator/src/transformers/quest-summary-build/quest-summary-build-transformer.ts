 ...
 *
 * - FLOW TYPE. A track only gets a row on a flow whose type it measures. Flowrider measures runtime
 *   flows alone, so an operational flow carries a siegemaster row and no flowrider row — printing a
 *   flowrider `outstanding` there would report work no Flowrider session will ever do.
 * - UNIT KIND. The off-map probe families are Siegemaster's charter and are absent from Flowrider's
 *   unit kinds, so they never land in Flowrider's numbers.
 ...
 *   walker added mid-walk did not exist while Flowrider was authoring and can never receive a
 *   Flowrider sign-off. `signoffTrackEligibilityStatics.byTrack.flowrider.observableOrigins` omits
 *   `siegemaster` for exactly that reason, and filtering on it is what keeps such an observable out
 *   of Flowrider's `outstanding` instead of parking it there forever. NO TIMESTAMP IS COMPARED —
 *   `at` records when a sign-off was written, not when a role's item completed, and ordering roles
 *   by wall-clock would break the moment a resumed session wrote out of order.
 *
 * `outstanding` IS TAKEN FROM `signoffFlowOutstandingTransformer`, the same call the signal-back
 * completion gate and `get-qa-checklist` both make. The number a reader sees here is therefore the
 * number that will refuse a `done`, rather than a second derivation that can drift from it.
 *
...
export const questSummaryBuildTransformer = ({ quest }: { quest: Quest }): QuestSummary => {
    ...
    signoffTrackContract.options
      ...
      .map((track) => {
        ...
        return {
          ...
          track,
          signoffField:
            track === 'flowrider' ? ('flowriderSignoff' as const) : ('siegemasterSignoff' as const),
          eligibleUnits: units
    ...
    flows: enumeratedFlows.map(({ flow }) => ({
      ...
      tracks: trackScopes
        ...
        .map((scope) => ({
          ...
          outstanding: signoffFlowOutstandingTransformer({ flow, track: scope.track }).length,
        })),
    })),
