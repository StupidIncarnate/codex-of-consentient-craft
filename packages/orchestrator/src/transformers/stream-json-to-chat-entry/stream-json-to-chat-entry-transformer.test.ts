...
describe('streamJsonToChatEntryTransformer', () => {
  describe('system init messages', () => {
    ...
    it('EDGE: {type: "system", subtype: "other"} => falls through to empty result', () => {
      const result = streamJsonToChatEntryTransformer({
        parsed: { type: 'system', subtype: 'other' },
      });
  ...
  describe('assistant messages', () => {
    ...
    it('EDGE: {type: "assistant", no message} => returns empty entries', () => {
      const result = streamJsonToChatEntryTransformer({ parsed: { type: 'assistant' } });
  ...
  describe('user messages with tool_result', () => {
    ...
    it('EDGE: {type: "user", no message} => returns empty entries', () => {
      const result = streamJsonToChatEntryTransformer({ parsed: { type: 'user' } });
  ...
  describe('unrecognized messages', () => {
    it('EDGE: {type: "unknown"} => returns empty result', () => {
      const result = streamJsonToChatEntryTransformer({ parsed: { type: 'unknown' } });
    ...
    it('EDGE: {no type field} => returns empty result', () => {
      const result = streamJsonToChatEntryTransformer({ parsed: { data: 'something' } });
    ...
    it('EDGE: {non-object input} => returns empty result', () => {
      const result = streamJsonToChatEntryTransformer({ parsed: null });
