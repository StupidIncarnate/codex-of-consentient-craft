...
describe('blightChecklistBuildTransformer', () => {
  describe('pairing: impl + test + proxy + stub', () => {
    it('VALID: {impl.ts + impl.test.ts + impl.proxy.ts + impl.stub.ts} => one group, 7 units, pairedFiles holds the three companions', () => {
      ...
      const stubPath = RepoRelativePathStub({
        value: 'packages/orchestrator/src/brokers/foo/foo-broker.stub.ts',
      });
      const { baseRef } = BlightChecklistStub();

      const result = blightChecklistBuildTransformer({
        changedFiles: [implPath, testPath, proxyPath, stubPath],
        baseRef,
      });
      ...
      expect(result.items).toStrictEqual([
        {
          ...
          implPath,
          concern: 'coverage',
          pairedFiles: [proxyPath, stubPath, testPath],
  ...
  describe('pairing: test-only diff pulls the impl path into scope', () => {
    ...
    it('VALID: {.proxy.tsx + .stub.ts changed, no markerless impl in the diff} => implPath is .tsx because a companion is .tsx', () => {
      const proxyTsxPath = RepoRelativePathStub({
        value: 'packages/web/src/widgets/bar/bar-widget.proxy.tsx',
      });
      const stubPath = RepoRelativePathStub({
        value: 'packages/web/src/widgets/bar/bar-widget.stub.ts',
      });
      const { baseRef } = BlightChecklistStub();

      const result = blightChecklistBuildTransformer({
        changedFiles: [stubPath, proxyTsxPath],
        baseRef,
      });
      ...
      expect(result.items[0]).toStrictEqual({
        ...
        concern: 'coverage',
        pairedFiles: [proxyTsxPath, stubPath],
