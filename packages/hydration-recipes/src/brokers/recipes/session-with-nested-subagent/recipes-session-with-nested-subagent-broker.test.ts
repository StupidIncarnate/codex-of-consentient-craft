...
import { recipesSessionWithNestedSubagentBrokerProxy } from './recipes-session-with-nested-subagent-broker.proxy';
...
describe('recipesSessionWithNestedSubagentBroker', () => {
  ...
  describe('refusals', () => {
    it('ERROR: {a guild the lane does not know} => throws naming it and listing what the lane knows', async () => {
      ...
      await expect(
        recipesSessionWithNestedSubagentBroker({
          context: RecipeContextStub({ apiBaseUrl: API, homePath: HOME }),
