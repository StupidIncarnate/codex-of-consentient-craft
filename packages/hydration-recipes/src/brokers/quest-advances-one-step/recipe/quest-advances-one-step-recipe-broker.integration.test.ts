...
import { questAdvancesOneStepRecipeBroker } from './quest-advances-one-step-recipe-broker';
...
describe('questAdvancesOneStepRecipeBroker', () => {
  describe('the manifest identity chunk 8 reads off this same export', () => {
    it('VALID: {} => carries its verbatim name, description, and a real inputs schema', () => {
      expect({
        recipeName: questAdvancesOneStepRecipeBroker.recipeName,
        description: questAdvancesOneStepRecipeBroker.description,
        hasInputs: questAdvancesOneStepRecipeBroker.inputs !== undefined,
      }).toStrictEqual({
