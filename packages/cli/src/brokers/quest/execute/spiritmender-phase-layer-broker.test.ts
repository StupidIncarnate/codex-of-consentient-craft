...
describe('spiritmenderPhaseLayerBroker', () => {
  describe('successful execution', () => {
    it('VALID: {all files fixed} => returns completed true', async () => {
      const { slotManagerProxy } = spiritmenderPhaseLayerBrokerProxy();
      ...
      const result = await spiritmenderPhaseLayerBroker({
        questFilePath,
        slotCount,
        timeoutMs,
        slotOperations,
  ...
  describe('multiple files with errors', () => {
    it('VALID: {multiple files all fixed} => returns completed true', async () => {
      const { slotManagerProxy } = spiritmenderPhaseLayerBrokerProxy();
