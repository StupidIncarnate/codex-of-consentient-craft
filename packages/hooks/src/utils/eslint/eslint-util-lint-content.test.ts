...
describe('eslintUtilLintContent', () => {
  describe('valid input', () => {
    it("VALID: {filePath: 'test.ts', content: 'const x = 1;'} => returns fixed content and results", async () => {
      ...
      const result = await eslintUtilLintContent({ filePath, content });
  ...
  describe('error handling', () => {
    ...
    it('ERROR: eslint crashes with exit code 2 and stderr => throws error', async () => {
      ...
      await expect(eslintUtilLintContent({ filePath, content })).rejects.toThrow(
