...
describe('eslintUtilRunTypescriptCheck', () => {
  describe('valid input', () => {
    it("VALID: {filePath: 'test.ts'} typescript check passes => returns hasErrors false", async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      const result = await eslintUtilRunTypescriptCheck({ filePath: 'test.ts' });
    ...
    it('VALID: typescript check with --noEmit flag => validates without output', async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      await eslintUtilRunTypescriptCheck({ filePath: 'src/example.ts' });
    ...
    it('VALID: exit code 0 with empty stdout and stderr => returns hasErrors false', async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      const result = await eslintUtilRunTypescriptCheck({ filePath: 'valid.ts' });
    ...
    it('VALID: non-zero exit code with error details => returns hasErrors true with details', async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      const result = await eslintUtilRunTypescriptCheck({ filePath: 'broken.ts' });
  ...
  describe('edge cases', () => {
    it('EDGE: empty file path => attempts typescript check', async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      const result = await eslintUtilRunTypescriptCheck({ filePath: '' });
    ...
    it('EDGE: process spawn timeout after 30 seconds => returns hasErrors true', async () => {
      ...
      const { eslintUtilRunTypescriptCheck } = await import('./eslint-util-run-typescript-check');
      const result = await eslintUtilRunTypescriptCheck({ filePath: 'large-file.ts' });
