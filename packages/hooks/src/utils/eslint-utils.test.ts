...
import { HookConfigLoader } from './hook-config-loader';

jest.mock('eslint');
jest.mock('./hook-config-loader');
...
describe('EslintUtils', () => {
  ...
  // Configuration management tests (moved from eslint-config.test.ts)
  describe('loadConfigByFile()', () => {
    ...
    it('VALID: {cwd: "/project", filePath: "test.ts"} => returns eslint config', async () => {
      ...
      const result = await EslintUtils.loadConfigByFile({
        cwd: '/project',
        filePath: 'test.ts',
      });
    ...
    it('VALID: same cwd called twice => returns cached config on second call', async () => {
      ...
      const result1 = await EslintUtils.loadConfigByFile({ cwd: '/test', filePath: 'file1.ts' });
      const result2 = await EslintUtils.loadConfigByFile({ cwd: '/test', filePath: 'file2.ts' });
    ...
    it('VALID: different cwd => calculates new config', async () => {
      ...
      const result1 = await EslintUtils.loadConfigByFile({ cwd: '/test1', filePath: 'file.ts' });
      const result2 = await EslintUtils.loadConfigByFile({ cwd: '/test2', filePath: 'file.ts' });
  ...
  describe('createFilteredConfig()', () => {
    ...
    it('VALID: {eslintConfig with rules, hookConfig} => returns filtered config', () => {
      ...
      const result = EslintUtils.createFilteredConfig({
        eslintConfig,
        hookConfig,
      });
