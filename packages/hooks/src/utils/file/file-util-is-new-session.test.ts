...
describe('fileUtilIsNewSession', () => {
  it("VALID: file doesn't exist => returns true", async () => {
    const fs = require('fs');
    const spy = jest.spyOn(fs, 'existsSync').mockReturnValue(false);

    const result = await fileUtilIsNewSession({ transcriptPath: '/test/nonexistent.txt' });
  ...
  it('VALID: file exists and size < 1024 => returns true', async () => {
    ...
    const result = await fileUtilIsNewSession({ transcriptPath: '/test/small.txt' });
  ...
  it('VALID: file exists and size >= 1024 => returns false', async () => {
    ...
    const result = await fileUtilIsNewSession({ transcriptPath: '/test/large.txt' });
  ...
  it('ERROR: file stat error => returns true', async () => {
    ...
    const result = await fileUtilIsNewSession({ transcriptPath: '/test/error.txt' });
