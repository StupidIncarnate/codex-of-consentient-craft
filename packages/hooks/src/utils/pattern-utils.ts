...
export const PatternUtils = {
  ...
  checkContent: ({
    ...
    const applicablePatterns = PatternUtils.filterPatternsForFile({
      patterns: ESCAPE_HATCH_PATTERNS,
      filePath,
    });
    ...
    for (const pattern of applicablePatterns) {
      ...
      if (stripStrings) {
        ...
        if (pattern.isCommentBased) {
          contentToCheck = TextUtils.stripStringLiterals({ content });
        } else {
          contentToCheck = TextUtils.stripCommentsAndStringLiterals({ content });
        }
      } else {
  ...
  buildErrorMessage: ({ violations }: { violations: string[] }) =>
    [
  ...
  processEscapeHatchisms: ({ content, filePath }: { content: string; filePath?: string }) => {
    const { violations } = PatternUtils.checkContent({ content, stripStrings: false, filePath });
    return violations.length > 0
      ? { found: true, message: PatternUtils.buildErrorMessage({ violations }) }
      : { found: false, message: '' };
  },

  processEscapeHatchismsInCode: ({ content, filePath }: { content: string; filePath?: string }) => {
    const { violations } = PatternUtils.checkContent({ content, stripStrings: true, filePath });
    return violations.length > 0
      ? { found: true, message: PatternUtils.buildErrorMessage({ violations }) }
      : { found: false, message: '' };
  },
  ...
  hasNewEscapeHatches: ({
    ...
    const { types: oldTypes } = PatternUtils.checkContent({
      content: oldContent,
      stripStrings: true,
      filePath,
    });
    const { types: newTypes } = PatternUtils.checkContent({
      content: newContent,
      stripStrings: true,
      filePath,
    });
  ...
  getNewEscapeHatchMessage: ({
    ...
    newContent: string;
    filePath?: string;
  }) => PatternUtils.processEscapeHatchismsInCode({ content: newContent, filePath }).message,
};
