import { eslintLintContentBroker } from './eslint-lint-content-broker';
import { ChildProcessMocker } from '@questmaestro/testing';
import * as childProcessSpawnPromiseModule from '../../../adapters/child-process/child-process-spawn-promise';

describe('eslintLintContentBroker', () => {
  describe('valid input', () => {
    it("VALID: {filePath: 'test.ts', content: 'const x = 1;'} => returns fixed content and results", async () => {
      ...
      const result = await eslintLintContentBroker({ filePath, content });
    ...
    it('VALID: eslint returns fixed output => returns fixed content', async () => {
      jest.spyOn(childProcessSpawnPromiseModule, 'childProcessSpawnPromise').mockResolvedValue({
        code: 0,
  ...
  describe('error handling', () => {
    ...
    it('ERROR: eslint crashes with exit code 2 and stderr => throws error', async () => {
      ...
      // Mock childProcessSpawnPromise directly to ensure the result is returned
      jest.spyOn(childProcessSpawnPromiseModule, 'childProcessSpawnPromise').mockResolvedValue({
        code: 2,
        stdout: '',
      ...
      await expect(eslintLintContentBroker({ filePath, content })).rejects.toThrow(
