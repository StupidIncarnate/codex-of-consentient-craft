...
ruleTester.run('enforce-project-structure', ruleEnforceProjectStructureBroker(), {
  valid: [
    ...
    {
      code: 'export const AuthLoginResponder = () => null;',
  ...
  invalid: [
    ...
    {
      code: 'import plugin from "@typescript-eslint/eslint-plugin";\nexport const typescriptEslintEslintPluginLoadAdapter = plugin;',
      filename:
        '/project/src/adapters/typescript-eslint-eslint-plugin/load/typescript-eslint-eslint-plugin-load-adapter.ts',
    ...
    {
      code: 'export function httpGetAdapterProxy() { return null; }',
      filename: '/project/src/adapters/http/get/http-get-adapter.proxy.ts',
      errors: [{ messageId: 'proxyMustBeArrowFunction' }],
    },
    // Proxy with class export
    {
      code: 'export class HttpGetAdapterProxy {}',
      filename: '/project/src/adapters/http/get/http-get-adapter.proxy.ts',
      errors: [{ messageId: 'proxyMustBeArrowFunction' }],
    },
    // Proxy with wrong export name (missing Proxy suffix)
    {
      code: 'export const httpGetAdapter = () => {};',
      filename: '/project/src/adapters/http/get/http-get-adapter.proxy.ts',
      errors: [{ messageId: 'invalidExportSuffix' }, { messageId: 'filenameMismatch' }],
    },
