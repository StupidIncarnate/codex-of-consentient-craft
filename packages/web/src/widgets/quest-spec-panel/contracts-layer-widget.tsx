...
import { collectUnlinkedContractsTransformer } from '@dungeonmaster/shared/transformers';
...
export const ContractsLayerWidget = ({
  ...
  const unlinkedContracts = collectUnlinkedContractsTransformer({ contracts });

  return (
    <Box data-testid="CONTRACTS_LAYER">
      <PlanSectionWidget
        title={CONTRACTS_LABEL}
        items={unlinkedContracts}
        editing={editing}
        onAdd={() => {
          onChange({
            contracts: [
        ...
        onRemove={(unlinkedIndex) => {
          const targetId = unlinkedContracts[unlinkedIndex]?.id;
          onChange({
            contracts: contracts.filter((c) => c.id !== targetId),
            toolingRequirements: tooling,
          });
        }}
        renderItem={(contract) => {
          const fullIndex = contracts.findIndex((c) => c.id === contract.id);
