...
export const pathDirnameAdapterProxy = (): {
  ...
  // each call dirname on a path that is itself the output of a prior mocked call (the broker's
  // own join result, or a config path just loaded), so there is no fixed argument to key on
  // across the composed proxies. `returns()` stays call-order-scoped (`onceFor([])`), matching
  // @dungeonmaster/shared's own path-dirname-adapter proxy, with a real passthrough default via
  // requireActual for any call nobody staged.
