...
describe('opCreateContract', () => {
  describe('valid create ops', () => {
    it('VALID: {ingredient: "quest", ref: "guild[0]/quest[2]", index: 2, ancestors: ["guild[0]"], fields: {title: "The running one"}} => returns the whole op', () => {
      expect(
        OpCreateStub({
          ingredient: 'quest',
          ref: 'guild[0]/quest[2]',
    ...
    it('VALID: {ancestors: []} => a top-level row has no ancestors', () => {
      expect(
        OpCreateStub({ ingredient: 'guild', ref: 'guild[0]', index: 0, ancestors: [] }),
      ).toStrictEqual({
        op: 'create',
        ingredient: 'guild',
        ref: 'guild[0]',
