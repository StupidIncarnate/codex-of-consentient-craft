 ...
 * whether or not that route used the framework's own `fetchPostAdapter`.
 *
 * USAGE:
 * routeFailureTransformer({ cause: new Error('connect ECONNREFUSED') });
 * // Returns { url: null, status: null, responseBody: null }
 */
...
export const routeFailureTransformer = ({ cause }: { cause: unknown }): RouteFailure => {
  if (
    ...
    return routeFailureContract.parse({ url: cause.url, status: cause.status, responseBody: null });
  }
