 ...
 *
 * USAGE:
 * pathToTreeRelativeTransformer({ filepath: FilePathStub({ value: '/mono/packages/hooks/src/adapters/fs/write-file/fs-write-file-adapter.ts' }) });
...
export const pathToTreeRelativeTransformer = ({ filepath }: { filepath: FilePath }): FilePath => {
  ...
  if (pkgMatch) {
    ...
    return filePathContract.parse(`${pkgName}/${afterSrc}`);
  }
  ...
  if (srcIndex >= 0) {
    return filePathContract.parse(pathStr.slice(srcIndex + SRC_SEGMENT.length));
  }

  // Relative path starting with `src/`  → strip the leading `src/`
  if (pathStr.startsWith('src/')) {
    return filePathContract.parse(pathStr.slice('src/'.length));
  }
