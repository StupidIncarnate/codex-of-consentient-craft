...
export const claudeCodeSubagentFindByWorkItemIdBrokerProxy = (): {
  setupHomeDir: (params: { path: string }) => void;
  setupSubagentDirFiles: (params: { files: readonly string[] }) => void;
  setupSubagentDirMissing: () => void;
  setupFileContents: (params: { filename: string; firstLine: string }) => void;
} => {
  ...
  return {
    ...
    setupFileContents: ({ filename, firstLine }: { filename: string; firstLine: string }): void => {
      readFileProxy.returns({
        filepath: PathSegmentStub({ value: filename }),
        contents: FileContentsStub({ value: firstLine }),
      });
    },
  };
};
