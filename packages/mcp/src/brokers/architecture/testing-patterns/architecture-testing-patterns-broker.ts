...
4. **Search dist/ for stale references:** If a contract schema changed, bash \`grep\` is blocked by this repo's own PreToolUse hook — use \`discover({ glob: 'packages/*/dist/**', grep: 'oldFieldName' })\` to reveal consumers that weren't rebuilt.
