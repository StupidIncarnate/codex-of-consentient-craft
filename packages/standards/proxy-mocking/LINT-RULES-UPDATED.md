...
### Rule 1: All Testable Files Must Have Proxy and Test Files

**Status:** ⚠️ UPDATED (expanded to all layers)

```typescript
// packages/eslint-plugin/src/rules/testable-must-have-proxy.ts
export const testableMustHaveProxy = {
    meta: {
        messages: {
            missingProxy: '{{filename}} is missing {{proxyFile}}. All testable files (adapters, brokers, transformers, guards, bindings, widgets, responders, flows, routes) must have a proxy file.',
            missingTest: '{{filename}} is missing {{testFile}}. All testable files must have a test file.'
        }
    },
    create(context) {
        return {
            Program(node) {
                ...
                // Match all testable file patterns
                ...
                const isTestable = testablePatterns.some(pattern => pattern.test(filename));

                if (isTestable && !filename.includes('.proxy.') && !filename.includes('.test.')) {
                    // Derive expected proxy and test file names
                    const proxyFile = filename.replace(/\.tsx?$/, '.proxy$&');
                    const testFile = filename.replace(/\.tsx?$/, '.test$&');

                    if (!fs.existsSync(proxyFile)) {
                        context.report({
                            node,
                            messageId: 'missingProxy',
                            data: {
                                filename: filename.split('/').pop(),
                                proxyFile: proxyFile.split('/').pop()
                            }
                        });
                    }
...
Every testable file must have:

1. **Proxy file** - Provides semantic setup helpers for tests
2. **Test file** - Tests the implementation

```typescript
// ❌ BAD - Missing proxy and test files
adapters / http / http - adapter.ts          // ❌ No proxy or test!

// ✅ GOOD - Complete trio
adapters / http / http - adapter.ts          // Implementation
adapters / http / http - adapter.proxy.ts    // Proxy for testing
adapters / http / http - adapter.test.ts     // Tests

// Applies to ALL layers:
brokers / user / create / user - create - broker.ts
brokers / user / create / user - create - broker.proxy.ts    // ✅ Required
brokers / user / create / user - create - broker.test.ts     // ✅ Required

transformers / format - name / format - name - transformer.ts
transformers / format - name / format - name - transformer.proxy.ts    // ✅ Required
transformers / format - name / format - name - transformer.test.ts     // ✅ Required

guards / has - permission / has - permission - guard.ts
guards / has - permission / has - permission - guard.proxy.ts    // ✅ Required
guards / has - permission / has - permission - guard.test.ts     // ✅ Required

bindings / use - user / use - user - binding.ts
bindings / use - user / use - user - binding.proxy.ts    // ✅ Required
bindings / use - user / use - user - binding.test.ts     // ✅ Required

widgets / user - card / user - card - widget.tsx
widgets / user - card / user - card - widget.proxy.ts    // ✅ Required
widgets / user - card / user - card - widget.test.tsx    // ✅ Required

responders / user / get / user - get - responder.ts
responders / user / get / user - get - responder.proxy.ts    // ✅ Required
responders / user / get / user - get - responder.test.ts     // ✅ Required

flows / user / user - flow.ts
flows / user / user - flow.proxy.ts    // ✅ Required
flows / user / user - flow.test.ts     // ✅ Required

routes / user / user - routes.ts
routes / user / user - routes.proxy.ts    // ✅ Required
routes / user / user - routes.test.ts     // ✅ Required
```

**Key insight:** Proxies are not just for adapters - they're for ALL testable layers. Even pure functions (transformers,
guards) need proxies to provide semantic setup helpers for higher-layer tests.
