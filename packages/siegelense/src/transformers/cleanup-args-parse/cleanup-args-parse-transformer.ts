 ...
 * `--json` and `--human` refuses, carrying the call's own refusal sentence verbatim from the MCP
 * registration this call replaces — cleanup "takes no input" (siegelense-tooling.md line 2409) —
 * AND the canonical positional-argument sentence its six siblings all carry, so a stray positional
 * argument here states both why nothing was expected and the general rule every other call states
