 ...
 * a nested `query` rather than flattened beside `isJson`, so the responder hands `pruneRunBroker`
 * that object whole and no broker ever sees a rendering opinion. Reach for this over `PruneQuery`
 * once the responder receives `--json`; `PruneQuery` is what the broker takes.
 *
 * USAGE:
