/**
 * PURPOSE: One row of `registry.json` — id, owner, quest, pid, pgids, specHash, ports, state and
 * lastBeat (spec line 134), the whole on-disk record for one instance, alive or gone. Every field
 * that can be absent is `.nullable()`, never `.optional()`: the row round-trips through
 ...
 * shape drift apart. A RESERVED instance is a row with `pid: null` and `bootedAtMs: null` — not a
 * sixth `InstanceState` member, because the five members of `InstanceState` are the closed set
 * `results` answers with (spec lines 2175-2181), and `capacity` still counts a reservation because a
 * reserved row is already sitting in this registry. `questId`/`guildId` are both null for an
 * `unowned/` instance. A reaped or pruned row is a TOMBSTONE — `state: 'pruned'` with `prunedAtMs`
 * and `prunedByRule` both set — so what is gone answers as gone, never as an empty list (spec line
 * 1666); `start` records the quest id, and that recorded id is how `prune` and `cleanup` later
 * resolve "still referenced" (spec line 1668).
 *
 * USAGE:
