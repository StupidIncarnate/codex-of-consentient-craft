...
describe('SiegelenseCapacityResponder', () => {
  describe('the bare form', () => {
    it('EMPTY: {specName: null, poolSize: null} => writes the CapacityAnswer as one JSON document', async () => {
  ...
  describe('both flags named', () => {
    it('VALID: {specName, poolSize: 3} => passes both through to the broker and writes its answer', async () => {
      const proxy = SiegelenseCapacityResponderProxy();
  ...
  describe('a spec nothing has ever run', () => {
    it('EMPTY: {profile: null} => writes the default-pair answer rather than refusing', async () => {
      const proxy = SiegelenseCapacityResponderProxy();
      const answer = CapacityAnswerStub({
        suggested: 2,
        profile: null,
