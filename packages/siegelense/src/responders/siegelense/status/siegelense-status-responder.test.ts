...
describe('SiegelenseStatusResponder', () => {
  describe('no instance named, empty fleet', () => {
    it('EMPTY: {instanceId: null, no instances} => writes the plain fleet-empty sentence', async () => {
  ...
  describe('an instance named, that id not in the registry', () => {
    it('EMPTY: {instanceId: inst_deadbeef, no instances} => writes a sentence naming that id, distinct from the fleet-empty sentence', async () => {
      const proxy = SiegelenseStatusResponderProxy();
      const instanceId = InstanceIdStub({ value: 'inst_deadbeef' });
      const answer = StatusAnswerStub({ instances: [] });
      proxy.stageAnswer({ answer });

      await SiegelenseStatusResponder({ instanceId });
  ...
  describe('no instance named, a live and a dead instance', () => {
    it('VALID: {instanceId: null} => writes the machine block, the monitored list, and one line per instance', async () => {
      const proxy = SiegelenseStatusResponderProxy();
  ...
  describe('a named dead instance', () => {
    it('VALID: {instanceId: inst_9b2c} => writes that instance in full', async () => {
      const proxy = SiegelenseStatusResponderProxy();
