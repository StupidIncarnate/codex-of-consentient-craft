 ...
 * stop here instead. Measured on quest a7520e60: two round reviewers ran a git-scoped ward moments
 * after pushing their own round, each got a full sweep with e2e (858s of one, 600s+ of the other,
 * both past the harness timeout and into background polling). Both reported the wide green as their
 * round's verdict.
 *
...
export const fileScopeEmptyStatics = {
  message:
    'ward: the file scope resolved to 0 source files, so NO checks ran. The working tree is clean (--uncommitted), this branch has committed nothing the origin default branch lacks (--committed), or the file list handed in was empty. This is an EMPTY run, not a green one. To check something, scope it yourself: npm run ward -- -- <files>',
} as const;
