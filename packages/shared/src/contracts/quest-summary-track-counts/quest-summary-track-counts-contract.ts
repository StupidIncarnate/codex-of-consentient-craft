 ...
 *
 * `id` IS THE TRACK, so `tracks` is an id-bearing array rather than a `Record<SignoffTrack, …>`.
 * Every other per-key collection on a quest is an id-bearing array for the same reason: the quest
 * deep-merge upserts array elements by `id` and replaces a plain object value wholesale, and a
 * reader that maps over `tracks` keeps working when a third track lands.
 *
