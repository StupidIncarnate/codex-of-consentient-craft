 ...
 * own remainder, a coverage row on the quest summary, an operation item's own work list. Its
 * sibling `signoffTrackContract` names the sign-off FIELDS instead, and is the one to reach for
 * wherever a value selects a COLUMN to read or write.
 *
 * USAGE:
 * signoffDenominatorTrackContract.parse('flowrider');
 * // Returns: SignoffDenominatorTrack enum value
 *
 * THE TWO LISTS ARE SEPARATE EVEN WHERE THEY AGREE. More than one role can WRITE one sign-off
 * field, over `packageTypes` that are disjoint — neither settles the other's units, and a role
