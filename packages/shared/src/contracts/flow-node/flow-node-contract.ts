 ...
 * hand every reader a node it cannot route. `.default([])` fails because it materialises an empty
 * array into EVERY node of every persisted quest.json on each re-parse, the mistake the sibling
 * `flowObservableContract` records a measured +116% file-size cost for. Required means every node on
 * disk carries a real value.
 *
