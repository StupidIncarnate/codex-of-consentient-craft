 ...
 * (`packageType`). Reach for this over a bare `packageNameContract` wherever a downstream decision
 * needs the package's kind — e2e eligibility, prompt routing — or its post-quest existence, which is
 * what the dependency graph is built from. A bare `packageNameContract` stays the right type for a
 * pointer that only has to name a package the reader can already look up on disk.
 *
 * USAGE:
...
export const questPackageEntryContract = z.object({
  ...
  packageType: packageTypeContract.describe(
    "The kind of package, stamped from the on-disk detector for 'edit' and 'delete'. A 'new' package has nothing on disk to detect, so its author declares it.",
  ),
  usedBy: z
