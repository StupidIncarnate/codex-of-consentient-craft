 ...
 * (watchers), joining on resolved literal file paths or <computed: brokerName> references.
 *
 * USAGE:
 ...
 *
 * WHEN-TO-USE: Building the file-bus-edges section of the project-map EDGES footer
...
export const fileBusEdgesLayerBroker = ({
  ...
  // writer: { filePath: ContentText, writerFile: AbsoluteFilePath }
  const writerEntries: { filePath: ContentText; writerFile: AbsoluteFilePath }[] = [];
  // watcher: { filePath: ContentText, watcherFile: AbsoluteFilePath }
  ...
  // Collect unique file path strings across both sides
