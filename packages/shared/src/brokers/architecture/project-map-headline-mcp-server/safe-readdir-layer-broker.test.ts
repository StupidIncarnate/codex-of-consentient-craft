...
const DIR_PATH = AbsoluteFilePathStub({ value: '/repo/packages/mcp/src/flows' });
...
describe('safeReaddirLayerBroker', () => {
  describe('directory exists', () => {
    it('VALID: {existing dir with files} => returns the directory entries', () => {
      ...
      const entries = [
        makeDirent({ name: 'architecture-flow.ts' }),
        makeDirent({ name: 'quest-flow.ts' }),
      ];
      proxy.returns({ entries });

      const result = safeReaddirLayerBroker({ dirPath: DIR_PATH });

      expect(result.map((e) => e.name)).toStrictEqual(['architecture-flow.ts', 'quest-flow.ts']);
    });
