 ...
 * // Returns the cap on rendered DEBT entries before that section truncates
 *
 ...
 * cost do NOT by themselves fit under the ceiling — 80 debt entries alone run about 45,000
 ...
 * summary carries is contract-governed to one short span: `questSummaryDebtContract.evidence` is a
 * test `file:line` plus the break that reds it (or the value read off the running system),
 * `questSummaryDebtContract.toSettle` is one routable instruction, `questNoteContract.summary` is
 * "the one line a reader scans", and an observable's `description` is a single GIVEN/WHEN/THEN
 * outcome. Capping prose per field would cut the very sentence this render exists to route.
 ...
 * mid-quest observables, 35 debt entries and 24 notes
 ...
 * which the loud notices and their exact dropped counts make visible; it can never let an unmarked
 * unit pass for a marked one. The section order — coverage, drift, debt, human check,
