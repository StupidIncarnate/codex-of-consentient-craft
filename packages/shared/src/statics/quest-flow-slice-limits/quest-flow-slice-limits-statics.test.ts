...
import { questFlowSliceLimitsStatics } from './quest-flow-slice-limits-statics';

const NOTICE_HEADROOM_CHARS = 1_000;

describe('questFlowSliceLimitsStatics', () => {
  describe('exported value', () => {
    it('VALID: {statics} => caps the whole slice render', () => {
      expect(questFlowSliceLimitsStatics).toStrictEqual({
  ...
  describe('the character ceiling clears the MCP verbatim-delivery budget', () => {
    ...
    // very act of announcing the cut is what spills the result to a file and hands the session a
    // path instead of its flow.
    it('VALID: {maxRenderChars} => leaves room for the truncation notice under maxVerbatimChars', () => {
      expect(
        mcpToolResultStatics.maxVerbatimChars - questFlowSliceLimitsStatics.maxRenderChars,
