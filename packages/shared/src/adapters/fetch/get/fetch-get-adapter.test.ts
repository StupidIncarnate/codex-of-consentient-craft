...
describe('fetchGetAdapter', () => {
  describe('successful requests', () => {
    it('VALID: {url, ok response with JSON} => returns parsed JSON', async () => {
      ...
      const url = 'http://localhost:4750/api/process/proc-foo';
      proxy.setupSuccess({ url, body: { status: 'running', queuePosition: 2 } });

      const result = await fetchGetAdapter({ url });
  ...
  describe('non-OK responses', () => {
    it('ERROR: {404 response with body} => throws with URL, status, and body text', async () => {
      const proxy = fetchGetAdapterProxy();
      const url = 'http://localhost:4750/api/process/proc-foo';
      proxy.setupNotOk({ url, status: 404, bodyText: 'Process not found' });

      await expect(fetchGetAdapter({ url })).rejects.toThrow(
    ...
    it('ERROR: {500 response with empty body} => throws with URL and status', async () => {
      const proxy = fetchGetAdapterProxy();
      const url = 'http://localhost:4750/api/process/proc-foo';
      proxy.setupNotOk({ url, status: 500, bodyText: '' });

      await expect(fetchGetAdapter({ url })).rejects.toThrow(
        'GET http://localhost:4750/api/process/proc-foo failed with status 500: ',
      );
    });
  });
  ...
  describe('invalid JSON responses', () => {
    it('ERROR: {ok response with non-JSON body} => throws with URL and body in message', async () => {
      const proxy = fetchGetAdapterProxy();
      const url = 'http://localhost:4750/api/process/proc-foo';
      proxy.setupInvalidJson({ url, bodyText: 'not json at all' });
