...
export const flowGraphToTextTransformer = ({
  ...
  for (const startNodeId of orderedNodeIds) {
    ...
    while (recursionStack.length > 0) {
      ...
      //
      // A package with no observable renders BARE rather than as `● 0` — where an absent marker
      // means nothing recorded. It also leaves the seam-declared-but-asserted-by-nothing case visible
