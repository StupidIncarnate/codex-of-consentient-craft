...
2. **Server: new endpoints + WS subscription handlers.** `quest-new-responder`, `quest-chat-responder`, `quest-clarify-responder`, new inbound `subscribe-quest`/`unsubscribe-quest`/`replay-quest-history` handlers (additive — old broadcast-to-all loop and `quest-by-session-request` stay parallel). API routes added.
