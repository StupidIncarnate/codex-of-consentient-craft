...
| 3.2 Move agent prompts into orchestrator         | DONE   | All 9 prompts in packages/orchestrator/src/statics/ (chaoswhisperer, pathseeker, codeweaver, lawbringer, siegemaster, spiritmender, quest-start, pathseeker-quest-review-minion, chaoswhisperer-gap-minion). roleToPromptTemplateTransformer routes by role. |
| 3.3 Update prompts: MCP → HTTP (curl/Bash)       | DONE   | 8/9 prompts fully migrated. chaoswhisperer-gap-minion has stale mcp__dungeonmaster__signal-back in tools frontmatter (prompt body uses HTTP). Fixed in Phase 4.                                                                                              |
