...
export class QuestStateBuilder {
  ...
  inQuestCreationState(status: QuestStatus = QuestStatus.ACTIVE, options?: StateOptions) {
    ...
    this.quest.phases.discovery.findings = {
      components: (options?.customComponents || [{ name: 'test-component', dependencies: [] }]).map(
        (c) => ({ name: c.name, dependencies: c.dependencies || [] }),
      ),
    ...
    if (status === QuestStatus.BLOCKED && options?.withBlockers) {
      this.addBlocker(
        ...
        options.errorMessage || 'Project missing required configuration',
      );
    }
  ...
  inTaskweaverState(status: QuestStatus = QuestStatus.ACTIVE, options?: StateOptions) {
    return this.inQuestCreationState(status, options);
  }

  // PATHSEEKER STATE
  inPathseekerState(status: PhaseStatus = PhaseStatus.COMPLETE, options?: StateOptions) {
    // Ensure quest creation happened first
    if (
      !this.stateHistory.includes('quest_creation') &&
      !this.stateHistory.includes('taskweaver')
    ) {
    ...
    if (
      currentStatus === PhaseStatus.NOT_STARTED &&
      (status === PhaseStatus.COMPLETE || status === PhaseStatus.BLOCKED)
    ) {
    ...
    if (status === PhaseStatus.COMPLETE) {
      ...
      this.addBlocker(
        'discovery_failed',
        options?.errorMessage || 'Unable to analyze codebase structure',
      );
  ...
  // CODEWEAVER STATE
  inCodeweaverState(status: PhaseStatus = PhaseStatus.COMPLETE, options?: StateOptions) {
    ...
    if (status === PhaseStatus.COMPLETE) {
      ...
      if (options?.partialOnly) {
        ...
        componentsToImplement = independentComponents.slice(
          0,
          Math.max(1, Math.floor(independentComponents.length / 2)),
        );
      } else {
  ...
  // LAWBRINGER STATE
  inLawbringerState(status: PhaseStatus = PhaseStatus.COMPLETE, options?: StateOptions) {
    ...
    if (status === PhaseStatus.COMPLETE) {
      ...
      let issues = options?.reviewIssues || [];

      if (
        !issues.length &&
        options?.withErrors &&
        this.quest.phases.implementation.components.length > 0
      ) {
      ...
      this.quest.phases.review.progress = options?.percentComplete
        ? `${options.percentComplete}%`
        : '50%';
    }
  ...
  inSiegemasterState(status: PhaseStatus = PhaseStatus.COMPLETE, options?: StateOptions) {
    ...
    if (status === PhaseStatus.COMPLETE) {
      ...
      this.quest.phases.gapAnalysis.analysisResults =
        this.quest.phases.implementation.components.map((comp) => ({
          component: comp.name,
      ...
      this.quest.agentReports.siegemaster.push({
        ...
        timestamp: new Date().toISOString(),
        fullReport: AgentReportTemplates.siegemaster(
          String(gapsFound),
          this.quest.phases.gapAnalysis.analysisResults || [],
        ),
      });
    ...
    this.addActivity(`Gap analysis ${status}`, 'siegemaster', {
      gapsFound:
        this.quest.phases.gapAnalysis.analysisResults?.reduce((sum, r) => sum + r.gapsFound, 0) ||
        0,
    });
  ...
  // SPIRITMENDER STATE
  inSpiritMenderState(resolved: boolean = true, options?: StateOptions) {
    // Create a blocker if none exist
    if (!this.quest.blockers || this.quest.blockers.length === 0) {
      this.quest.status = QuestStatus.BLOCKED;
      this.addBlocker('ward_failure', options?.errorMessage || 'ESLint errors: Unused variables');
    }
    ...
    if (resolved) {
      ...
      if (
        this.quest.phases.implementation.components.every(
  ...
  // ABANDONED STATE
  inAbandonedState(reason: string = 'User requested abandonment') {
  ...
  private addActivity(action: string, agent: string, details: unknown) {
