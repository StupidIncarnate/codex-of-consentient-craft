...
`tests/e2e/web/harness/claude-mock/` provides a fake `claude` CLI for Playwright E2E tests. The real server flow runs
end-to-end (spawn → parse stream-json → extract session → broadcast WebSocket) but without hitting the real LLM.
