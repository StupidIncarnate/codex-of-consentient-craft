...
### report — chunk 7
RESULT:
  - fetchGetWithStatusAdapter resolves with {status: 500, ok: false, body: <parsed>} for a 500 instead of throwing. — yes — "VALID: {500 JSON response} => returns status 500 with parsed body without throwing" passes: result is {status: 500, ok: false, body: {error: 'server exploded'}}.
  - It resolves with {status: 200, ok: true, body: <parsed>} for a 200. — yes — "VALID: {200 JSON response} => returns status, ok, and parsed body" passes: result is {status: 200, ok: true, body: {key: 'value'}}.
  - A connection-level failure still REJECTS, so the caller can tell "answered badly" from "never answered". — yes — "ERROR: {network failure} => rejects" passes: fetchGetWithStatusAdapter against a StartEndpointMock networkError() endpoint rejects, matched against /fetch/iu.
  - It sends GET with an Accept: application/json header and no body, and the URL it is given reaches globalThis.fetch unchanged — no query string is appended. — yes — read off disk: the fetch call is globalThis.fetch(url, {method: 'GET', headers: {Accept: 'application/json'}}) with no body key and url passed straight through with no concatenation or query-string logic anywhere in the file. Every test in the suite registers StartEndpointMock.listen({method: 'get', url: '/test/endpoint'}) and stages/asserts against that same literal url with no query string, and each one resolves against the staged response rather than the handler's unconfigured-500 fallback — which is only possible if the adapter requested exactly GET /test/endpoint.
  - A non-JSON body comes back as raw text rather than throwing, matching the POST adapter's behaviour exactly. — yes — "VALID: {non-JSON text body} => returns raw text body" passes: a respondRaw({status: 500, body: 'plain failure'}) response returns {status: 500, ok: false, body: 'plain failure'}, via the same text-then-JSON.parse-with-catch-fallback shape fetch-post-with-status-adapter.ts uses.
FILES:    ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.ts, ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.proxy.ts, ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.test.ts
EVIDENCE:
  - UNITS: none (per this chunk's own UNITS line) — no unit row to prove, so evidence is the full INTENT-line red/green cycle below instead.
  - "VALID: {200 JSON response} => returns status, ok, and parsed body" against the empty shell (fixed {status: 200, ok: true, body: null} return, ignoring the request): expected {status: 200, ok: true, body: {key: 'value'}}, received {status: 200, ok: true, body: null}.
  - "VALID: {500 JSON response} => returns status 500 with parsed body without throwing" against the shell: expected {status: 500, ok: false, body: {error: 'server exploded'}}, received {status: 200, ok: true, body: null}.
  - "VALID: {non-JSON text body} => returns raw text body" against the shell: expected {status: 500, ok: false, body: 'plain failure'}, received {status: 200, ok: true, body: null}.
  - "EMPTY: {204 empty body} => returns body null" against the shell: expected {status: 204, ok: true, body: null}, received {status: 200, ok: true, body: null} — the status field disagreed even though body/ok happened to match, so this case still proves the assertion reaches real code.
  - "ERROR: {network failure} => rejects" against the shell: expected the promise to reject, received "promise resolved instead of rejected — Resolved to value: {body: null, ok: true, status: 200}".
  - After writing the real implementation, the identical scoped ward run went green on all 5 cases with no other change to the test file.
USAGES:   discover({grep: "fetchGetWithStatusAdapter"}) returns hits only in this chunk's own three files under packages/web/src/**, plus an unrelated same-named adapter and its caller in packages/hooks/src/** (a different package's own fetch-get-with-status-adapter.ts under adapters/fetch/get-with-status/, wired to hook-post-ask-question-responder — coincidental name reuse across packages, not a collision, and outside this round's package scope). Nothing under packages/web/src/** imports this adapter yet; per this chunk's own NOTES, chunk 10's broker (a later wave, not yet built) is the only planned consumer.
GOTCHAS:
  - The shell's first draft used void url; to silence the unused-parameter lint while stubbing out the real fetch call; that tripped no-void during the scoped ward run (a genuine lint FAIL, not test red). Left it in place through the red-capture step since it did not affect the red evidence, then removed it entirely in the real implementation (the real body uses url in the fetch() call, so no placeholder was needed). Final ward run has zero lint findings.
  - This chunk's own PURPOSE header deliberately does NOT restate the return shape or throwing behaviour in the PURPOSE paragraph itself (per get-syntax-rules' "MUST NOT" list) — it leads with when to reach for this adapter over its two nearest siblings (fetchGetAdapter, fetchPostWithStatusAdapter) instead, and defers the concrete shape to the USAGE comment beneath it, which is where get-syntax-rules says a return-shape example belongs.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.ts ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.proxy.ts ./packages/web/src/adapters/fetch/get-with-status/fetch-get-with-status-adapter.test.ts — green — lint PASS (3/3 files), typecheck PASS (7287/7287 files across 13 packages), unit PASS (1/1 files, 360 discovered), integration PASS (1/1 files, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 8
RESULT:
  - healthBadgeTitleTransformer returns exactly 'No response from server' for the offline branch with offlineCause 'unreachable'. — yes — test "VALID: {offlineCause: unreachable} => returns \"No response from server\"" passes; result is the literal 'No response from server'.
  - It returns exactly 'Server returned 500' for offlineCause 'server-error' with offlineStatusCode 500, and exactly 'Server returned 503' for 503 — the code is read from the state, never hardcoded. — yes — both cases pass: offlineStatusCode 500 → 'Server returned 500', offlineStatusCode 503 → 'Server returned 503'; the code comes from `${String(offlineStatusCode)}` in a template literal, never a literal digit in the source.
  - It returns exactly 'No heartbeat for 30 seconds' for offlineCause 'silence'. — yes — test passes; result is the literal 'No heartbeat for 30 seconds'.
  - For the checking, online and degraded branches it returns the plain state word from healthBadgeStatics, so the title is always a string and the widget never has to branch on undefined. — yes — three tests pass: checking → 'CHECKING', online → 'ONLINE', degraded → 'DEGRADED', each read straight off healthBadgeStatics.<key> with no undefined branch.
  - It does NOT import health-badge-label-transformer. — yes — read the file's import block: only display-label-contract, health-badge-state-contract (type-only) and health-badge-statics are imported.
FILES:    ./packages/web/src/transformers/health-badge-title/health-badge-title-transformer.ts, ./packages/web/src/transformers/health-badge-title/health-badge-title-transformer.test.ts
EVIDENCE:
  - check-offline-title-unreachable — offlineCause 'unreachable': expected "No response from server", received "offline" (shell returning badgeState.state verbatim).
  - check-offline-title-server-error — offlineCause 'server-error', offlineStatusCode 500: expected "Server returned 500", received "offline".
  - check-offline-title-silence — offlineCause 'silence': expected "No heartbeat for 30 seconds", received "offline".
  - (supporting, not a named unit) offlineCause 'server-error', offlineStatusCode 503: expected "Server returned 503", received "offline".
  - (supporting) offlineCause 'server-error', no offlineStatusCode: expected "Server returned" (prefix alone), received "offline".
  - (supporting) state 'checking': expected "CHECKING", received "checking".
  - (supporting) state 'online': expected "ONLINE", received "online".
  - (supporting) state 'degraded': expected "DEGRADED", received "degraded".
USAGES:   discover({grep: "healthBadgeTitleTransformer"}) — the only hits are this chunk's own two files, plus one unrelated PRE-EXISTING comment mention inside health-badge-statics.ts's PURPOSE docstring (chunk 2's file, landed already, just naming this transformer in prose). Nothing imports the transformer yet — chunk 12's widget is the eventual and only planned consumer per this chunk's own NOTES, and it has not landed.
GOTCHAS:
  - The net-new empty shell could not just return a fixed string: an unused `badgeState` destructured param trips both `@typescript-eslint/no-unused-vars` and TS6133, which blocked the test suite from even loading (no wrong-value red reachable). Used `displayLabelContract.parse(badgeState.state)` as the shell instead — still zero title-composition logic — which produced genuine wrong-value reds ("offline"/"checking"/"online"/"degraded" instead of the expected titles) on all 8 cases.
  - ESLint's `default-case` and `@typescript-eslint/consistent-return` both require an explicit `default:` arm on each switch even though the union is exhaustively narrowed by TypeScript. The outer switch's default cannot reference `badgeState.state` there — TS narrows `badgeState` to `never` in that branch, so `badgeState.state` is TS2339 — so it throws a fixed string. The inner (offlineCause) default CAN reference `offlineCause` inside `String(...)`, because `String()`'s parameter type is `any` and never triggers a property-access check. Both are unreachable at runtime given the contract's own exhaustive branches; flagged in case a reviewer expects the two defaults to read symmetrically.
  - Did not import health-badge-label-transformer, per NOTES — chunk 5 lands that file in this same wave.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/transformers/health-badge-title/health-badge-title-transformer.ts ./packages/web/src/transformers/health-badge-title/health-badge-title-transformer.test.ts — green — lint PASS (2/2), typecheck PASS (1115/1115 files), unit PASS (1/1 file, 362 discovered), integration PASS (1/1 file, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 5
RESULT:
  - formatUptimeTransformer({uptimeSeconds}) returns exactly '3h 12m' for 11520 and exactly '25h 1m' for 90061 — no day unit at any input. — yes — VALID tests for 11520 and 90061 both pass with toBe('3h 12m') / toBe('25h 1m').
  - formatUptimeTransformer returns '0h 0m' for 0 and '0h 1m' for 61, so a fresh server renders the same shape as an old one. — yes — EDGE tests for 0 and 61 both pass with toBe('0h 0m') / toBe('0h 1m').
  - healthBadgeLabelTransformer returns exactly 'ONLINE 3h 12m' for the online branch with uptimeSeconds 11520, exactly 'DEGRADED' for the degraded branch, exactly 'OFFLINE' for every offline branch whatever its cause, and exactly 'CHECKING' for the checking branch. — yes — six passing tests: online (11520), degraded, offline x3 (unreachable/server-error/silence), checking — each toBe on the exact word.
  - The degraded and offline labels have no trailing space and nothing appended — asserted with toBe on the whole string, not a prefix match. — yes — every degraded/offline assertion is `toBe('DEGRADED')` / `toBe('OFFLINE')` on the full returned string (Jest's toBe would fail on any extra trailing character), not a substring/prefix matcher.
  - healthPayloadToBadgeStateTransformer maps status 'ok' to the online branch and status 'degraded' to the degraded branch, carries uptimeSeconds straight through, and returns a state with NO lastHeartbeatAt when it is handed null. — yes — "status ok, lastHeartbeatAt: null" toStrictEqual {state:'online', uptimeSeconds:11520} and "status degraded, lastHeartbeatAt: null" toStrictEqual {state:'degraded'}, neither carrying a lastHeartbeatAt key.
  - Given a lastHeartbeatAt it returns a state carrying exactly that timestamp. — yes — "status ok, lastHeartbeatAt: given" and "status degraded, lastHeartbeatAt: given" both toStrictEqual an object carrying `lastHeartbeatAt: ISO_TIMESTAMP` exactly.
FILES:    ./packages/web/src/transformers/format-uptime/format-uptime-transformer.ts, ./packages/web/src/transformers/format-uptime/format-uptime-transformer.test.ts, ./packages/web/src/transformers/health-badge-label/health-badge-label-transformer.ts, ./packages/web/src/transformers/health-badge-label/health-badge-label-transformer.test.ts, ./packages/web/src/transformers/health-payload-to-badge-state/health-payload-to-badge-state-transformer.ts, ./packages/web/src/transformers/health-payload-to-badge-state/health-payload-to-badge-state-transformer.test.ts
EVIDENCE:
  - formatUptimeTransformer VALID {uptimeSeconds: 11520} => '3h 12m': empty-shell (`displayLabelContract.parse('')`) run printed `Expected: "3h 12m" / Received: ""`.
  - formatUptimeTransformer VALID {uptimeSeconds: 90061} => '25h 1m': same shell printed `Expected: "25h 1m" / Received: ""`.
  - formatUptimeTransformer EDGE {uptimeSeconds: 0} => '0h 0m' and EDGE {uptimeSeconds: 61} => '0h 1m': same shell printed `Received: ""` against each expected string.
  - healthBadgeLabelTransformer online/degraded/offline(x3 causes)/checking: empty-shell (`displayLabelContract.parse('')`) run printed `Received: ""` against each of 'ONLINE 3h 12m', 'DEGRADED', 'OFFLINE' (x3), 'CHECKING' — six distinct wrong-value reds.
  - healthPayloadToBadgeStateTransformer {status: ok, lastHeartbeatAt: null}: empty-shell (always returns `{state:'checking'}`) run printed `- "state": "online" / - "uptimeSeconds": 11520 / + "state": "checking"`.
  - healthPayloadToBadgeStateTransformer {status: ok, lastHeartbeatAt: given}: same shell printed the same 'checking' mismatch against the full {state:'online', uptimeSeconds:11520, lastHeartbeatAt} object.
  - healthPayloadToBadgeStateTransformer {status: degraded, lastHeartbeatAt: null}: same shell printed `- "state": "degraded" / + "state": "checking"`.
  - healthPayloadToBadgeStateTransformer {status: degraded, lastHeartbeatAt: given}: same shell printed the mismatch against {state:'degraded', lastHeartbeatAt}.
USAGES:   discover({grep: "formatUptimeTransformer|healthBadgeLabelTransformer|healthPayloadToBadgeStateTransformer"}) returns hits only inside this chunk's own three source/test file pairs — health-badge-label-transformer.ts imports formatUptimeTransformer internally (as the chunk's own NOTES describes), and nothing else in the repo references any of the three yet. Chunks 10, 11 and 12 (the plan's named future consumers of the payload and label transformers) have not landed — confirmed by the same grep and by git status showing no such files present.
GOTCHAS:
  - Neither UptimeSeconds nor IsoTimestamp is exported as its own standalone contract in this package — both brands are declared inline (UptimeSeconds inside both shared's health-status-payload-contract.ts and chunk 3's health-badge-state-contract.ts; IsoTimestamp only inside health-badge-state-contract.ts, optional on every branch it appears on). So formatUptimeTransformer's `uptimeSeconds` param is typed via `HealthStatusPayload['uptimeSeconds']` (an indexed-access read off an existing contract type, not a hand-rolled ad-hoc type), and health-payload-to-badge-state-transformer.ts declares a local `type IsoTimestamp = NonNullable<Extract<HealthBadgeState, {state:'online'}>['lastHeartbeatAt']>` for the same reason — derived from the real contract, never a fresh interface. In the test file this made constructing a definitely-non-optional branded ISO literal awkward (the field is `.optional()` on every branch, so even a narrowed stub read is `IsoTimestamp | undefined`), so I used a plain string literal with `as never`, matching the exact pattern this chunk's MIRROR file (duration-display-transformer.test.ts) already uses for a WorkItem timestamp field with no standalone stub of its own.
  - Wave-2 sibling chunks were actively writing beside this one during the run (visible as untracked dirs in `git status`: guards/is-heartbeat-silent, adapters/fetch/get-with-status, transformers/health-badge-title, plus modified adapters/rxjs and state/web-socket-channel files) — none overlap this chunk's FILES, and health-badge-label-transformer.ts deliberately does NOT import health-badge-title-transformer.ts per the plan's same-wave warning.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/transformers/format-uptime/format-uptime-transformer.ts ./packages/web/src/transformers/format-uptime/format-uptime-transformer.test.ts ./packages/web/src/transformers/health-badge-label/health-badge-label-transformer.ts ./packages/web/src/transformers/health-badge-label/health-badge-label-transformer.test.ts ./packages/web/src/transformers/health-payload-to-badge-state/health-payload-to-badge-state-transformer.ts ./packages/web/src/transformers/health-payload-to-badge-state/health-payload-to-badge-state-transformer.test.ts — green — lint PASS (6/6 files), typecheck PASS (7295/7295 files across 13 packages), unit PASS (3/3 files, 364 discovered), integration PASS (3/3 files, 8 discovered — the same test files are discovered under both check types, matching the doubled-discovery behavior chunk 3's report already noted for this package), e2e skip (no e2e-relevant file in scope).

### report — chunk 6
RESULT:
  - isHeartbeatSilentGuard returns false when now is exactly 29000 ms after lastHeartbeatAt. — yes — test "EDGE: {elapsed: 29000ms} => returns false" passes; against the shell it caught a wrong-value red (Expected: false, Received: true).
  - It returns true when now is exactly healthBadgeStatics.silenceThresholdMs after lastHeartbeatAt — the boundary is inclusive, and the test names 30000 rather than reading the static. — yes — test "EDGE: {elapsed: 30000ms} => returns true" passes; the test computes now from a literal +30000 on a fixed ISO anchor, never reading healthBadgeStatics.silenceThresholdMs, so a change to the static would red this test.
  - It returns false when lastHeartbeatAt is absent, so a badge that has never seen a frame never flips on silence. — yes — test "EMPTY: {lastHeartbeatAt: undefined} => returns false" passes; against the shell it caught a wrong-value red (Expected: false, Received: true).
  - It reads no clock of its own: now is a parameter, so every case is provable with no timer. — yes — read the file: the function body has no Date.now(), setTimeout or setInterval call; now is the only time source and is passed in by the caller on every branch.
FILES:    ./packages/web/src/guards/is-heartbeat-silent/is-heartbeat-silent-guard.ts, ./packages/web/src/guards/is-heartbeat-silent/is-heartbeat-silent-guard.test.ts
EVIDENCE:
  - check-still-online-before-threshold (29000ms case): first run against a shell returning lastHeartbeatAt !== undefined || now !== undefined || true (always true) printed Expected: false / Received: true — a genuine wrong-value red, not an import/type error.
  - check-offline-after-silence (30000ms case): that same always-true shell happened to satisfy this assertion by coincidence, so a second shell was staged — Boolean(lastHeartbeatAt) && now !== undefined && false (always false) — and rerun scoped to just this test: printed Expected: true / Received: false, a genuine wrong-value red. Restored the real implementation immediately after, in the same turn; git status on the file shows only the final version.
  - The two EMPTY-input tests (lastHeartbeatAt undefined, now undefined) are not separate UNITS but back the fourth INTENT line; both also printed Expected: false / Received: true against the first (always-true) shell.
USAGES:   discover({grep: "isHeartbeatSilentGuard"}) returns hits only in this chunk's own two files (the guard and its test). Chunk 11's binding (a later wave, not yet built) is the plan's named consumer; nothing else in the round currently imports it.
GOTCHAS:
  - Guard params must be typed from HealthBadgeState per the chunk's NOTES ("take THAT, rather than restating an ISO string type of your own") rather than a fresh z.string().datetime().brand<'IsoTimestamp'>(). Since HealthBadgeState is a discriminated union where the 'checking' branch omits lastHeartbeatAt entirely, direct HealthBadgeState['lastHeartbeatAt'] indexing does not type-check; used type HealthBadgeOnlineState = Extract<HealthBadgeState, { state: 'online' }> and indexed off that instead — the same narrowing-alias pattern already used in this package's widgets/transformers for ChatEntry (e.g. chat-entry-list-widget.tsx:36-37).
  - In the test file, testing patterns ban importing types from contracts, which rules out the same Extract-off-HealthBadgeState approach there. Followed this package's own precedent instead: duration-display-transformer.test.ts (the chunk's sibling MIRROR-adjacent file) passes raw ISO literals as never for VALID cases, not just INVALID ones, to sidestep exactly this branded-value plumbing — did the same here for the lastHeartbeatAt literals.
  - now is typed as a plain number, unbranded — get-syntax-rules' "Function Arguments vs Return Types" section states inputs (unlike returns) may use raw primitives, and config-dungeonmaster-broker.ts:81 sets allowPrimitiveInputs: true repo-wide, so this is not a ban-primitives violation despite the chunk's own NOTES phrasing it as "number-branded-per-repo-convention".
  - ward's scoped run discovers this same .test.ts under both "unit" and "integration" check types, both green — the same doubled-listing behavior chunk 3's report already flagged for this package's contract tests; not something this chunk introduced.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/guards/is-heartbeat-silent/is-heartbeat-silent-guard.ts ./packages/web/src/guards/is-heartbeat-silent/is-heartbeat-silent-guard.test.ts — green — lint PASS (2/2), typecheck PASS (1121/1121 files), unit PASS (1/1 file, 364 discovered), integration PASS (1/1 file, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 9
RESULT:
  - webSocketChannelState.healthStatus$() exists and a frame of {type: 'health-status', payload: {status, uptimeSeconds, version}, timestamp} delivered through the channel proxy reaches a subscriber carrying the PARSED payload, not the raw record. — yes — "VALID: {health-status ws message} => healthStatus$ emits parsed payload synchronously" passes: captured[0] toStrictEqual's HealthStatusPayloadStub({uptimeSeconds: 11520}), a branded/parsed object, not the raw JSON.
  - A health-status frame whose payload is missing uptimeSeconds reaches no subscriber, because the branch safeParses through healthStatusPayloadContract the way every other branch parses its own payload. — yes — "INVALID: {health-status ws message missing uptimeSeconds} => healthStatus$ emits nothing" passes: captured toStrictEqual's [].
  - The new branch sits inside dispatchInbound and pushes synchronously — the subscriber's callback has run by the time deliverMessage returns. — yes — the same VALID test reads captured[0] immediately after proxy.deliverMessage(...) returns, with nothing awaited in between; it is populated.
  - The health-status subject survives triggerClose followed by triggerReconnect: a subscriber taken before the close still receives the first frame after the reopen, with nothing re-subscribed. — yes — "VALID: {subscriber taken before close} => receives the first health-status frame delivered after reconnect" subscribes once, calls proxy.triggerClose() then proxy.triggerReconnect() with no second .subscribe() call, delivers a frame, and captured[0] toStrictEqual's the frame's payload.
  - rxjsSubjectAdapter exposes a reader over Subject.observed, and webSocketChannelState reports false for health-status subscribers before anything subscribes, true while one subscription is live, and false again after it unsubscribes. — yes — rxjs-subject-adapter.test.ts's two new cases pass (observed() false with no subscriber; true while subscribed, false after unsubscribe), and web-socket-channel-state.test.ts's "hasHealthStatusSubscribers tracks each state" passes all three: beforeSubscribe false, whileSubscribed true, afterUnsubscribe false.
  - Every existing branch of dispatchInbound and every existing observable getter is byte-unchanged, and web-socket-channel-state.test.ts's existing cases still pass. — yes — git diff --stat on web-socket-channel-state.ts shows 13 insertions/1 deletion, the one deletion being the single import line replaced by a two-name import (same statement, additive); no line inside an existing branch or getter body changed. Scoped ward's unit/integration run shows the whole @dungeonmaster/web package green (24/24 unit files, 31/31 integration files), including every pre-existing case in this test file.
FILES:
  - ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.ts (edited: +2 lines — observed added to the return type and returned object)
  - ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.proxy.ts (edited: +3 lines — observed added in parity to create<T>()'s declared and returned shape)
  - ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.test.ts (edited: +2 new it cases for observed())
  - ./packages/web/src/state/web-socket-channel/web-socket-channel-state.ts (edited: +13/-1 — healthStatusSubject added to internalState, healthStatus$ + hasHealthStatusSubscribers getters added, one new branch appended to dispatchInbound)
  - ./packages/web/src/state/web-socket-channel/web-socket-channel-state.test.ts (edited: +4 new it cases across 3 new/extended describe blocks)
FILES-UNCHANGED (in chunk's FILES list, left untouched, ward-verified green):
  - ./packages/web/src/state/web-socket-channel/web-socket-channel-state.proxy.ts — every capability the new tests need (deliverMessage, triggerClose, triggerReconnect) already existed; healthStatus$() and hasHealthStatusSubscribers() are called directly on the real webSocketChannelState, so no proxy method was missing.
EVIDENCE:
  - check-payload-updates-badge — first run with the branch structurally present but the .next() push withheld (shell): expect(captured[0]).toStrictEqual(HealthStatusPayloadStub({uptimeSeconds: 11520})) printed Expected: {"status": "ok", "uptimeSeconds": 11520, "version": "0.1.0"} / Received: undefined — a genuine wrong-value red, not a type/import error.
  - check-reconnect-resumes — same shell state: expect(captured[0]).toStrictEqual(HealthStatusPayloadStub({uptimeSeconds: 11580})) printed Expected: {"status": "ok", "uptimeSeconds": 11580, "version": "0.1.0"} / Received: undefined.
  - check-subscription-released — two independent reds, one per layer: rxjsSubjectAdapter().observed() with a hardcoded false shell printed Expected: true / Received: false on the "true while subscribed" assertion; webSocketChannelState.hasHealthStatusSubscribers() with its own hardcoded false shell printed the same Expected: true / Received: false on its whileSubscribed assertion.
USAGES:   discover({grep: "healthStatus$|hasHealthStatusSubscribers", glob: "packages/web/src/**"}) — hits only inside this chunk's own two files (web-socket-channel-state.ts and its test). discover({grep: "rxjsSubjectAdapter", glob: "packages/web/src/**"}) — hits in this chunk's own adapter/proxy/test files, plus web-socket-channel-state.ts's existing ten subjects (unchanged call sites) and web-socket-channel-state.proxy.ts's existing rxjsSubjectAdapterProxy() construction call (unchanged). Chunk 11 (not yet built, a later wave) is the only planned future consumer of healthStatus$() and hasHealthStatusSubscribers() per this chunk's own NOTES.
GOTCHAS:
  - Getting a genuine wrong-value red for the dispatchInbound branch could NOT use the "omit the branch entirely" shape the get-worker-information "existing code must behave differently" row describes literally, because that leaves the newly-imported healthStatusPayloadContract unused — a TS6133 that is not scoped to this file: since every test file in the package transitively imports web-socket-channel-state.ts, the whole unit/integration suite (23+ files) failed to even load. Fixed by keeping the branch's safeParse call in place (so the import is used and the file compiles) and withholding only the final .next() push as the shell — still a genuine behavioural gap, still a wrong-value red, without the compile-cascade side effect. Worth knowing for any later chunk adding a new envelope-type branch to this same dispatchInbound function.
  - SubjectAdapter<T> (web-socket-channel-state.ts:45) is ReturnType<typeof rxjsSubjectAdapter<T>>, exactly as chunk 9's own NOTES said — adding observed to the adapter's return type gave all eleven existing subjects (chatOutputSubject through the new healthStatusSubject) the same reader automatically, with no further edit to any of the other ten call sites.
  - Did not touch openConnection's reconnect timer or onClose handler at all — the design decision ("a socket reconnect needs no special handling") and this chunk's own NOTES both say to prove the module-level subject survives a reconnect rather than build for it, and the reconnect test passes against the unmodified reconnect code.
  - rxjs-subject-adapter.proxy.ts's create<T>() is not actually exercised by any test today (neither its own file's tests nor web-socket-channel-state.proxy.ts call .create()) — updated it in parity per this chunk's NOTES anyway, since its declared return type would otherwise drift from the adapter's.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.ts ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.proxy.ts ./packages/web/src/adapters/rxjs/subject/rxjs-subject-adapter.test.ts ./packages/web/src/state/web-socket-channel/web-socket-channel-state.ts ./packages/web/src/state/web-socket-channel/web-socket-channel-state.proxy.ts ./packages/web/src/state/web-socket-channel/web-socket-channel-state.test.ts — green — lint PASS (6/6 files), typecheck PASS (1121/1121 files across 13 packages), unit PASS (24/24 files, 364 discovered), integration PASS (31/31 files, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 10
RESULT:
  - healthStatusGetBroker() issues exactly one GET against webConfigStatics.api.routes.healthStatus per call, with no query string — asserted from the endpoint's own recorded request count, not from a spy on fetch. — yes — test "VALID: {single call} => issues exactly one GET against the health-status endpoint" passes: proxy.getRequestCount() toBe(1). Read off disk: the URL is passed from webConfigStatics.api.routes.healthStatus straight into fetchGetWithStatusAdapter with no string concatenation anywhere in the file, so no query string is ever appended; the ward run's network log during the red-capture pass also recorded the request as bare `GET /api/health/status` with no query string on every case.
  - A 200 carrying {status: 'ok', uptimeSeconds: 11520, version: '1.4.0'} returns the online branch with uptimeSeconds 11520 and no lastHeartbeatAt. — yes — test "VALID: {200 body status ok, uptimeSeconds 11520} => ..." passes: result toStrictEqual({state: 'online', uptimeSeconds: 11520}) — no lastHeartbeatAt key present.
  - A 200 carrying status 'degraded' returns the degraded branch, with no uptimeSeconds on the returned object at all. — yes — test "VALID: {200 body status degraded} => ..." passes: result toStrictEqual({state: 'degraded'}).
  - A 500 returns the offline branch with offlineCause 'server-error' and offlineStatusCode 500, and does not throw. — yes — test "ERROR: {500 response} => ..." passes with no thrown error: result toStrictEqual({state: 'offline', offlineCause: 'server-error', offlineStatusCode: 500}).
  - A network error returns the offline branch with offlineCause 'unreachable', and does not throw. — yes — test "ERROR: {network failure} => ..." passes with no thrown error: result toStrictEqual({state: 'offline', offlineCause: 'unreachable'}).
  - A 200 whose body fails healthStatusPayloadContract returns the offline branch with offlineCause 'server-error' rather than throwing into the binding. — yes — test "INVALID: {200 body missing uptimeSeconds} => ..." passes with no thrown error: result toStrictEqual({state: 'offline', offlineCause: 'server-error'}).
FILES:
  - ./packages/web/src/brokers/health-status/get/health-status-get-broker.ts (new)
  - ./packages/web/src/brokers/health-status/get/health-status-get-broker.proxy.ts (new)
  - ./packages/web/src/brokers/health-status/get/health-status-get-broker.test.ts (new)
EVIDENCE:
  - check-seed-request-issued — the red-first shell still made the real network call on every path (only the RETURN value was replaced with a constant), so this unit's own test happened to pass against the shell too — no wrong-value red to report for this one specifically. Proven instead by the final green run (proxy.getRequestCount() toBe(1)) plus the code read above.
  - check-offline-on-500 — shell (always returns {state: 'checking'} regardless of outcome) printed: `- "offlineCause": "server-error" / - "offlineStatusCode": 500 / - "state": "offline" / + "state": "checking"` — a genuine wrong-value red.
  - check-offline-on-network-failure — same shell printed: `- "offlineCause": "unreachable" / - "state": "offline" / + "state": "checking"` — a genuine wrong-value red.
  - (supporting, not a named unit but part of this chunk's own INTENT) the online-branch case printed `- "state": "online" / - "uptimeSeconds": 11520 / + "state": "checking"`; the degraded-branch case printed `- "state": "degraded" / + "state": "checking"`; the invalid-body case printed `- "offlineCause": "server-error" / - "state": "offline" / + "state": "checking"` — three more genuine wrong-value reds, all against the same shell.
USAGES: discover({grep: "healthStatusGetBroker", glob: "packages/web/src/**"}) returns hits only inside this chunk's own three files. Nothing else in the package imports healthStatusGetBroker or healthStatusGetBrokerProxy yet — chunk 11's binding (a later wave, not yet built) is the plan's named and only consumer, and its own proxy is expected to delegate to this chunk's proxy method names.
GOTCHAS:
  - The proxy's setup surface landed as `setupSeed({payload})` (taking a full HealthStatusPayload built via HealthStatusPayloadStub) plus `setupServerError()`, `setupUnreachable()` and `setupInvalidBody({body})`, rather than a pair of `setupOnlineSeed`/`setupDegradedSeed` methods each taking loose {uptimeSeconds, version} fields. A loose-fields signature needs its params typed against the real branded `HealthStatusPayload['uptimeSeconds']`/`['version']`, and a test passing plain numeric/string literals against that type fails typecheck (confirmed: TS2322 "number is not assignable to number & BRAND<'UptimeSeconds'>" on a first attempt) — passing a whole stub-built payload sidesteps that without reaching for `as never`. Flagging this so chunk 11's binding proxy (or whoever composes this proxy next) calls `setupSeed({payload: HealthStatusPayloadStub({status: 'ok', ...})})` for the online/degraded cases rather than looking for split setup methods.
  - Getting a genuine wrong-value red for this brand-new file needed the same shape chunk 9's report already flagged for this package: a shell that still CALLS every dependency (fetchGetWithStatusAdapter, healthStatusPayloadContract.safeParse, healthPayloadToBadgeStateTransformer) so no import goes unused and the suite can load, but always RETURNS the same wrong constant ({state: 'checking'}) regardless of what those calls produced. Restored the real branching logic in the same turn once the reds were captured; git diff on the file at report time is the final version only.
  - Tried scoping the try/catch tightly around just the fetchGetWithStatusAdapter call (via `let result: FetchWithStatusResult;` assigned inside try, read after) so a hypothetical downstream parse bug could never get silently mislabeled as "offline/unreachable". Reverted: this repo's `@typescript-eslint/init-declarations` lint rule requires a `let` to be initialized on its declaration line, so an uninitialized-then-assigned-in-try declaration fails lint. Kept the wider try/catch (wrapping the whole body) that chunk 10's own NOTES describes, which is also what the mirror pattern already used precedent-free — noting this in case a later chunk hits the same lint wall reaching for the same tighter-scope idiom.
  - On the 200-with-unparsable-body path, offlineStatusCode is deliberately OMITTED rather than set to 200 (the field is optional on the offline branch) — the request DID reach the server and answered 2xx, only the body was bad, so reporting a status code here would let chunk 8's title transformer render a misleading "Server returned 200".
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/brokers/health-status/get/health-status-get-broker.ts ./packages/web/src/brokers/health-status/get/health-status-get-broker.proxy.ts ./packages/web/src/brokers/health-status/get/health-status-get-broker.test.ts — green — lint PASS (3/3 files), typecheck PASS (7298/7298 files across 13 packages), unit PASS (1/1 file, 365 discovered), integration PASS (1/1 file, 8 discovered — same doubled-discovery behavior chunk 3/5/6's reports already flagged for this package's contract-adjacent tests), e2e skip (no e2e-relevant file in scope).

### report — chunk 11
RESULT:
  - useHealthStatusBinding returns the checking branch before the seed resolves, and the seed's result after. — yes — test "VALID: {mount} => badgeState starts as the checking branch, then becomes the seed result": `currentState().badgeState` is `{state: 'checking'}` immediately after render, then `{state: 'online', uptimeSeconds: 11520}` once the staged seed resolves.
  - It calls the seed broker exactly once per mount: the endpoint's recorded request count is 1 after mount and still 1 after three delivered heartbeat frames. — yes — test "VALID: {mount, then a re-render}": `proxy.getRequestCount()` is `1` after a `rerender()`. Test "VALID: {three delivered heartbeats}": `proxy.getRequestCount()` is still `1` after three `deliverHeartbeat` calls.
  - A delivered health-status frame replaces the returned state, carrying that frame's uptimeSeconds and a lastHeartbeatAt. — yes — same test: immediately after the first `deliverHeartbeat({status: 'degraded'})`, `currentState().badgeState` is `{state: 'degraded', lastHeartbeatAt: '2026-07-28T10:00:00.000Z'}` — read within the same `act()` that delivered the frame, with nothing awaited in between.
  - Two frames carrying uptimeSeconds 11520 then 11580 leave uptimeSeconds 11580. — yes — test "VALID: {two heartbeats carrying 11520 then 11580}": final `badgeState` is `{state: 'online', uptimeSeconds: 11580, lastHeartbeatAt: '2026-07-28T10:00:00.000Z'}`.
  - At 29 seconds after the last frame the state is still the online branch from that frame; at 30 seconds it is the offline branch with offlineCause 'silence'. — yes — test "EDGE: {29 seconds of silence...}": `badgeState` is `{state: 'online', uptimeSeconds: 11520, lastHeartbeatAt: '2026-07-28T10:00:00.000Z'}` after `jest.advanceTimersByTime(29000)`. Test "EDGE: {30 seconds of silence...}": `badgeState` is `{state: 'offline', offlineCause: 'silence', lastHeartbeatAt: '2026-07-28T10:00:00.000Z'}` after `jest.advanceTimersByTime(30000)`.
  - After triggerClose then triggerReconnect, the first delivered frame moves the state out of offline with no retry call. — yes — test "VALID: {socket closes and reconnects}": `badgeState.state` is `'offline'` before, `'online'` after `triggerClose()` + `triggerReconnect()` + one `deliverHeartbeat`, with `proxy.getRequestCount()` still `1` throughout (no second seed request fired).
  - Mounting makes webSocketChannelState.hasHealthStatusSubscribers() true; unmounting makes it false. — yes — test "VALID: {mount then unmount}": `webSocketChannelState.hasHealthStatusSubscribers()` is `true` after mount + seed resolution, `false` after `unmount()`.
  - A frame delivered after unmount leaves the last returned state untouched and records zero console.error calls. — yes — test "VALID: {frame delivered after unmount}": `badgeState` after a post-unmount `deliverHeartbeat` call equals the state captured just before `unmount()`, and `proxy.hadNoConsoleErrors()` is `true`.
  - retry() issues exactly one further seed request, and a 200 with status ok moves the state from offline to the online branch carrying its uptime. — yes — test "VALID: {retry after an offline seed}": `proxy.getRequestCount()` goes from `1` to `2` after `currentState().retry()`, and `badgeState` ends at `{state: 'online', uptimeSeconds: 11520}`.
FILES:
  - ./packages/web/src/bindings/use-health-status/use-health-status-binding.ts
  - ./packages/web/src/bindings/use-health-status/use-health-status-binding.proxy.ts
  - ./packages/web/src/bindings/use-health-status/use-health-status-binding.test.ts
  ...
  - ./packages/web/src/statics/health-badge/health-badge-statics.test.ts
EVIDENCE:
  - All 11 tests (10 covering the 9 INTENT lines plus one extra branch-coverage case) were run against an empty-shell `useHealthStatusBinding` returning a fixed `{badgeState: {state: 'checking'}, retry: () => undefined}`. Every test produced a genuine wrong-value red: e.g. "VALID: {mount}" printed `Object { - "state": "online", - "uptimeSeconds": 11520, + "state": "checking" }`; the two silence-boundary EDGE tests printed the full expected online/offline objects (including `lastHeartbeatAt`) against `+ "state": "checking"` — proving the fake-clock + toISOString-mock plumbing in the proxy actually reaches the assertion rather than short-circuiting. After restoring the real implementation, one test's own assertion was wrong (expected `{state: 'degraded'}` with no `lastHeartbeatAt`, but the binding correctly re-stamps `lastHeartbeatAt` on every heartbeat-delivered branch, degraded included) — fixed the assertion, not the code, and it went green with the rest.
USAGES: `discover({grep: "useHealthStatusBinding", glob: "packages/web/src/**"})` returns hits only inside this chunk's own three files — nothing else in the round has landed to consume it yet (chunk 12's widget, a later wave, is the named future consumer). `discover({grep: "healthBadgeStatics", glob: "packages/web/src/**"})` confirmed the only whole-object `toStrictEqual` pin on the statics object is its own colocated test (already updated); the guard, both transformers, and this binding all read named keys off it, none of which broke from the addition.
GOTCHAS:
  - The transformer chunk 5 landed (`healthPayloadToBadgeStateTransformer`) types `lastHeartbeatAt` as an UNEXPORTED local `IsoTimestamp` alias, so the binding cannot import that type name to construct a fresh branded value from `new Date().toISOString()`. Solved by calling the transformer with `lastHeartbeatAt: null` (its own null path already omits the key), then re-parsing the whole result through `healthBadgeStateContract.parse({...baseState, lastHeartbeatAt: new Date().toISOString()})` — `.parse()`'s input type accepts a raw string for a branded field, so this re-brands without an `as` cast and without duplicating the online/degraded branch logic the transformer owns.
  - The silence tick reads `lastHeartbeatAt` off the CURRENT state via `'lastHeartbeatAt' in current ? current.lastHeartbeatAt : undefined` rather than a separate ref — every branch that has ever received a heartbeat already carries the field (online, degraded, and offline-from-silence all declare it optionally), and a state that has never seen one (checking, or offline-from-a-failed-seed) naturally has no key, so the guard receives `undefined` and correctly returns `false`. This means a badge that starts offline on a failed seed and NEVER gets a heartbeat stays offline-for-that-reason rather than being silently reclassified as offline-for-silence — covered by the new "EDGE: {silence tick fires with no heartbeat ever delivered}" test.
  - `healthBadgeStatics.silenceTickMs` did not exist on disk — chunk 2's own report explicitly declined to add it, deferring the decision to whichever chunk needed it first (its NOTES: "if you add it, say so in your report and keep the whole-object toStrictEqual honest"). Added it here as `1000` (the value this chunk's own NOTES assumed), updated the colocated whole-object test in the same edit, and marked it ADDED below.
  - The pre-edit lint hook blocks the AST pattern `const someName = (async)? () => {...}` when it is a bare local variable inside another function (flagged as "Nested functions are forbidden") — but the SAME shape wrapped in `useCallback(...)` is not flagged (confirmed against the already-committed `useRateLimitsBinding`'s own `refresh`). The binding's `retry` is a `useCallback`, and the mount effect calls `retry()` directly instead of a second bare `const seed = async () => {...}` — doubling as both the mount-time seed and the click-driven retry.
  - `jest.useFakeTimers({now: ...})` has to be seeded at the SAME instant `Date.prototype.toISOString` is mocked to return (`'2026-07-28T10:00:00.000Z'`), or `Date.now()` at heartbeat-delivery time is the REAL wall-clock instant the test happened to run at (today) while `lastHeartbeatAt` is a fixed constant a month or more in the past — making the guard read as already-silent from the very first tick. The proxy's `installSilenceClock()` method installs both together so a boundary test never has to reason about this itself. Both fake-timer boundary tests are written as fully SYNCHRONOUS (`it(..., () => {...})`, no `async`/`await`) precisely so the mount's own pending seed promise (staged with `setupServerError()` for hygiene, but never actually awaited) has no microtask turn to resolve on and race the timer-driven assertions.
  - Two `ban-primitives` violations surfaced on the proxy's first draft (`heartbeatDeliveredAt(): string` and `getConsoleErrorCallCount(): number`) — return types must be branded even in `.proxy.ts` files. Replaced both with semantic, non-primitive-returning methods instead: `installSilenceClock()`/`restoreRealClock()` (own the fake-timer setup so the test never needs the raw timestamp at all) and `hadNoConsoleErrors(): boolean`.
  - `jest/prefer-ending-with-an-expect` fired on four tests whose last statement was a helper call (`testingLibraryWaitForAdapter`, `unmount()`/`restoreRealClock()`) rather than a literal `expect(...)`, even though each already asserted inside a callback or an earlier line. Fixed by matching the established MIRROR pattern (`useRateLimitsBinding.test.ts`'s last case) — an explicit `expect(...)` as the test's true final statement, reordering `unmount()`/`restoreRealClock()` to run BEFORE it rather than after (safe: `result.current` retains its last value after unmount).
MARKERS:  ADDED: healthBadgeStatics.silenceTickMs (1000) — the binding's own silence-tick poll period, floated as optional in chunk 2's NOTES and explicitly deferred by chunk 2's own report to whichever chunk needed it first. Added here with the colocated whole-object test kept in sync.
WARD:     npm run ward -- -- ./packages/web/src/statics/health-badge/health-badge-statics.ts ./packages/web/src/statics/health-badge/health-badge-statics.test.ts ./packages/web/src/bindings/use-health-status/use-health-status-binding.ts ./packages/web/src/bindings/use-health-status/use-health-status-binding.proxy.ts ./packages/web/src/bindings/use-health-status/use-health-status-binding.test.ts — green — lint PASS (5/5 files), typecheck PASS (7301/7301 files across 13 packages), unit PASS (5/5 files, 366 discovered), integration PASS (5/5 files, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 12
RESULT:
  - The badge renders one element carrying data-testid HEALTH_BADGE in every one of the four states. — yes — the DOM captured in the shell-red run and the final green run both show one `<button data-testid="HEALTH_BADGE">` with no conditional around it; `healthBadgeStatics.testId` is the only source of the literal.
  - It reads exactly CHECKING between mount and the first seed response, and never after one has arrived. — yes — "pending state VALID: {mount}" asserts `hasBadgeText({text: 'CHECKING'})` true synchronously right after render (before any await), then asserts the seed result replaces it.
  - Given a seed body of {status: 'ok', uptimeSeconds: 11520, version: '1.4.0'} it reads exactly 'ONLINE 3h 12m'; given uptimeSeconds 90061 it reads exactly 'ONLINE 25h 1m'. — yes — "online label VALID" and "online label EDGE: {90061}" both pass with `toBe(true)` on the exact string.
  - Given a seed body with status 'degraded' it reads exactly DEGRADED, with nothing after the word. — yes — "degraded label VALID" passes; `hasBadgeText` compares the whole textContent with `===`, so a trailing character would fail it.
  - It reads exactly OFFLINE when the seed responds 500, and exactly OFFLINE when the seed never reaches the server. — yes — "offline label ERROR: {500}" and "offline label ERROR: {network failure}" both pass.
  - Its title attribute reads exactly 'No response from server', 'Server returned 500' and 'No heartbeat for 30 seconds' in those three cases respectively. — yes — "offline title" describe block's three cases all pass, the third via a fake-clock silence tick after a delivered online heartbeat.
  - The badge is an enabled control in its OFFLINE state: it is not disabled, and a click on it reaches the binding's retry. — yes — "offline is clickable VALID" asserts `isBadgeEnabled()` true, then clicks and asserts the request count moves from 1 to 2.
  - Clicking it while OFFLINE issues one further seed request, and a 200 with status ok changes the text from OFFLINE to ONLINE with its uptime appended. — yes — "retry VALID: {click while offline}" (count 1→2) and "retry VALID: {200 retry with status ok}" (text OFFLINE→'ONLINE 3h 12m') both pass.
  - Two delivered frames move the rendered text from 'ONLINE 3h 12m' to 'ONLINE 3h 13m'. — yes — "live heartbeats VALID" delivers uptimeSeconds 11520 then 11580 and asserts the text ends at 'ONLINE 3h 13m'.
FILES:
  - ./packages/web/src/widgets/health-badge/health-badge-widget.tsx (new)
  - ./packages/web/src/widgets/health-badge/health-badge-widget.proxy.tsx (new)
  - ./packages/web/src/widgets/health-badge/health-badge-widget.test.tsx (new)
EVIDENCE:
  - All 13 test cases (10 named units plus the pending/live-heartbeat/degraded supporting cases) were run against a SHELL widget that still called both transformers (so their imports stayed used and the file kept compiling) but rendered the fixed text 'SHELL' and title 'SHELL' unconditionally. Every case printed a genuine wrong-value red: `Expected: true / Received: false` on the `hasBadgeText`/`hasBadgeTitle`/`isBadgeEnabled` boolean-comparison assertions, with the captured DOM in the failure output showing `<button data-testid="HEALTH_BADGE" title="SHELL">SHELL</button>` in place of every expected state's text/title. check-offline-is-clickable and check-retry-refetches went red via `waitFor` TIMEOUT (the OFFLINE text never appeared, since the shell always reads SHELL) rather than a same-tick mismatch — still a genuine red reaching real code, not an import/type error. Restored the real implementation in the same turn; `git status`/`git diff` on the three files show only the final version, no shell code left in the tree.
  - check-offline-title-server-error's shell-red capture is the clearest single example: `Expected: true / Received: false` at test.tsx:139, with the rendered DOM inline in the failure showing `title="SHELL"` where `'Server returned 500'` was expected.
USAGES:   discover({grep: "HealthBadgeWidget", glob: "packages/web/src/**"}) returns hits only inside this chunk's own three files (the widget, its proxy and its test). Nothing else in the round has landed to consume it — chunk 13's AppWidget mount (a later wave, not yet built) is the plan's only named future consumer, and its own NOTES say it constructs HealthBadgeWidgetProxy and reads whichever of its setup methods it needs.
GOTCHAS:
  - Declaring `getBadgeText(): string | null` / `getBadgeTitle(): string | null` on the proxy tripped ban-primitives ("Raw string type is not allowed") on the FIRST draft, even though this is a proxy file. Switched to the repo's established boolean-comparison shape instead — `hasBadgeText({text})` / `hasBadgeTitle({text})` returning `boolean`, matching `DispatchToggleWidgetProxy.hasToggleLabel` and `PixelBtnWidgetProxy.hasLabel` exactly — rather than exposing the raw DOM string.
  - `jest/prefer-ending-with-an-expect` fired on 11 of the 13 original test bodies whose last statement was `await waitFor(() => { expect(...) })` rather than a literal `expect(...)` at the test's own top level, even though the callback already asserted. Fixed by adding a trailing top-level `expect(...)` restating the same assertion after every such `waitFor` (the same redundant-trailing-expect shape chunk 11's binding test already uses), and by reordering the silence-title test so `proxy.restoreRealClock()` runs BEFORE its final `expect(...)` rather than after (mirroring chunk 11's own GOTCHAS on this exact rule).
  - The proxy's setup methods are named for scenarios per this chunk's own NOTES — `setupOnlineSeed({uptimeSeconds})`, `setupDegradedSeed()`, `setupServerError()`, `setupUnreachable()`, `deliverHeartbeat({payload})` — rather than delegating the binding proxy's raw `setupSeed({payload})` name straight through. `setupOnlineSeed`/`setupDegradedSeed` take a plain `number`/no args and build the `HealthStatusPayloadStub` internally (StubArgument unbrands `uptimeSeconds` and `status` to raw primitives, so no branded-type plumbing was needed here, unlike chunk 10's broker-proxy GOTCHAS on the same stub).
  - The badge never sets an HTML `disabled` attribute in any state — chunk 12's own NOTES explicitly forbid gating the click on state, so `isBadgeEnabled()` (`element instanceof HTMLButtonElement && !element.disabled`) is trivially true in every state; the assertion still proves the observable (a native, undisabled `<button>`) rather than assuming it.
  - Did not reuse PixelBtnWidget, per NOTES: it carries its own `data-testid="PIXEL_BTN"` and no `title` prop, so wrapping it would either lose the badge's own testid or land the title on the wrong element.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/widgets/health-badge/health-badge-widget.tsx ./packages/web/src/widgets/health-badge/health-badge-widget.proxy.tsx ./packages/web/src/widgets/health-badge/health-badge-widget.test.tsx — green — lint PASS (3/3 files), typecheck PASS (1130/1130 files), unit PASS (1/1 file, 367 discovered), integration PASS (1/1 file, 8 discovered), e2e skip (no e2e-relevant file in scope).

### report — chunk 13
RESULT:
  - An element with data-testid HEALTH_BADGE is in the DOM when AppWidget renders at '/', and still in the DOM after navigating to the session-view route. — yes — new test "health badge mounting VALID: {mount at /} => HEALTH_BADGE testid is in the DOM" and "VALID: {navigate to session-view route} => HEALTH_BADGE testid stays in the DOM" both pass: `proxy.isHealthBadgeVisible()` (queries `screen.queryByTestId(healthBadgeStatics.testId)`) is `true` at mount and again after the guild → sessions → session-item navigation lands on the readonly session view.
  - It renders inside the logo row's right-hand flex cell, as a sibling of RATE_LIMITS_STACK's container, and neither the LOGO_LINK's centering nor the row's existing layout changes. — yes — read off disk: `<HealthBadgeWidget />` and `<RateLimitsStackWidget />` are the only two children of the same right-hand cell `<div style={{flex: '1 1 0', minWidth: 0, display: 'flex', justifyContent: 'flex-end', gap: LOGO_ROW_RIGHT_CELL_GAP}}>`; `git diff` on app-widget.tsx is 4 insertions/0 deletions — the left spacer div, the `<Link data-testid="LOGO_LINK">` block and every other property on the right-hand cell are byte-unchanged, with only a new `gap` property added alongside the existing four.
  - Every existing case in app-widget.test.tsx still passes, with AppWidgetProxy constructing HealthBadgeWidgetProxy so the badge's seed request finds a staged endpoint. — yes — scoped ward's unit run is green with no failures; every one of the pre-existing 23 `it()` cases in app-widget.test.tsx (guild list, session nav, guild creation, logo nav, error/edge cases) passed unmodified, and `AppWidgetProxy` now calls `HealthBadgeWidgetProxy()` (unassigned, no setup needed) before returning, which registers the health-status MSW handler so the badge's mount-time GET resolves instead of hitting `onUnhandledRequest:'error'`.
  - app-widget.integration.test.tsx still proves exactly ONE WebSocket is opened, now with the health binding mounted alongside chat, queue and rate-limits. — yes — the single case's title, comments and mounted-binding set were extended (not duplicated) to mount `useHealthStatusBinding()` as a fourth `testingLibraryRenderHookAdapter`, with a fourth MSW handler staged for `webConfigStatics.api.routes.healthStatus`; `expect(socketConstructions).toStrictEqual([true])` still passes — the health binding subscribes to the same `webSocketChannelState` singleton rather than opening a second socket.
FILES:    ./packages/web/src/widgets/app/app-widget.tsx, ./packages/web/src/widgets/app/app-widget.proxy.tsx, ./packages/web/src/widgets/app/app-widget.test.tsx, ./packages/web/src/widgets/app/app-widget.integration.test.tsx
EVIDENCE:
  - UNITS: check-badge-rendered → app-widget.tsx. Wrote both new test cases and the implementation in the same pass by mistake, then corrected the ordering before reporting: temporarily reverted app-widget.tsx to drop the `HealthBadgeWidget` import, the `LOGO_ROW_RIGHT_CELL_GAP` constant and the `<HealthBadgeWidget />` render (git-diff-clean revert, confirmed by `git diff` showing no changes), and re-ran `npm run ward -- --only unit --onlyTests "HEALTH_BADGE|health badge mounting" -- ./packages/web/src/widgets/app/app-widget.test.tsx` against the unmodified file.
  - Genuine wrong-value red, both new cases: `FAIL "AppWidget health badge mounting VALID: {mount at /} => HEALTH_BADGE testid is in the DOM"` and `FAIL "AppWidget health badge mounting VALID: {navigate to session-view route} => HEALTH_BADGE testid stays in the DOM"`, both `Error: expect(received).toBe(expected) // Object.is equality` on `expect(proxy.isHealthBadgeVisible()).toBe(true)` — the assertion ran, queried the real DOM, and found no HEALTH_BADGE element (received false), not an import or type error. Confirmed via the ward detail network log that neither test's mount triggered any GET to `/api/health/status` at all in the reverted state, since nothing rendered `HealthBadgeWidget` to call the binding.
  - Restored the exact intended diff (re-applied the same three edits), confirmed via `git diff -- packages/web/src/widgets/app/app-widget.tsx` matching the final 4-insertion/0-deletion diff quoted in RESULT line 2, then reran the full scoped ward over all four FILES — green, both new cases passing along with all pre-existing ones.
USAGES:   discover({grep: "AppWidget", glob: "packages/web/src/**"}) — the only consumer outside this chunk's own files is `responders/app/layout/app-layout-responder.ts` (`export const AppLayoutResponder = AppWidget;`) and its proxy (`AppWidgetProxy()`), neither of which reads or passes any prop AppWidget's signature still doesn't have — both untouched. discover({grep: "HealthBadgeWidget", glob: "packages/web/src/**"}) — hits only in this chunk's own two files (app-widget.tsx, app-widget.proxy.tsx) plus chunk 12's own health-badge-widget.tsx/.proxy.tsx/.test.tsx, which this chunk did not touch. Nothing else in the round or the package imports either name.
GOTCHAS:
  - `HealthBadgeWidgetProxy()` is called and discarded (no assignment, no methods surfaced on `AppWidgetProxy`'s own return object) — matching how `RateLimitsStackWidgetProxy()` is already called in this same file. None of app-widget's own test cases need a specific badge state; merely constructing the proxy chain registers the health-status MSW handler via `StartEndpointMock.listen`, which is what NOTES meant by "every existing AppWidget case throws the moment the badge's seed request finds no staged endpoint" — an unaddressed `StartEndpointMock.listen()` still answers (just with an unconfigured-response body), it is the ABSENCE of any registered handler for that URL that throws under `onUnhandledRequest:'error'`.
  - `isHealthBadgeVisible()` reads `healthBadgeStatics.testId` (imported from the real statics file chunk 2 landed) rather than hardcoding the literal `'HEALTH_BADGE'`, unlike this proxy's other hand-rolled selectors (`'QUEST_QUEUE_BAR'`, `'LOGO_LINK'`) which predate the badge and have no statics source of their own to read.
  - Gap value (12px, extracted to `LOGO_ROW_RIGHT_CELL_GAP`) and the badge-before-rate-limits child ordering were both judgment calls NOTES left open ("give the cell a gap rather than adding margin to the badge", no value or order specified). 12 matches this package's own convention for a bar-level horizontal gap (quest-queue-bar-widget.tsx's outer row, queue-page-widget.tsx, guild-session-list-widget.tsx all use `gap: 12`, vs. `8` for tight icon/text clusters).
  - app-widget.integration.test.tsx's health binding (`useHealthStatusBinding`) is left mounted (never unmounted) at test end, same as the other three bindings already in this file — its silence-tick `setInterval` keeps running on a real clock, but the package's own `jest.setup.js` calls `jest.useRealTimers()` in `afterEach` and the base jest config sets `forceExit: true`, so this does not hang or leak into later test files.
  - Ward's lint `--fix` auto-collapsed `isHealthBadgeVisible`'s arrow body from two lines to one during the final scoped run (visible as an on-disk change reported mid-tool-call); left as ward wrote it — purely a formatting normalization, same logic.
MARKERS:  none
WARD:     npm run ward -- -- ./packages/web/src/widgets/app/app-widget.tsx ./packages/web/src/widgets/app/app-widget.proxy.tsx ./packages/web/src/widgets/app/app-widget.test.tsx ./packages/web/src/widgets/app/app-widget.integration.test.tsx — green — lint PASS (4/4 files), typecheck PASS (7304/7304 files across 13 packages), unit PASS (3/3 files, 367 discovered), integration PASS (7/7 files, 8 discovered), e2e skip (no e2e-relevant file in scope).
